...
from ..settings import DEFAULT_CURRENCY, STRIPE_SECRET_API_KEY
from . import (
    PAYMENTS_OUTSIDE_PLATFORM,
    ChargeStatus,
    GatewayError,
    PaymentError,
    TransactionKind,
...
from .gateways.stripe.stripe_api import retrieve_txn
...
def create_payment(
    ...
    if gateway in PAYMENTS_OUTSIDE_PLATFORM:
        defaults["payment_method_type"] = PAYMENTS_OUTSIDE_PLATFORM[gateway]
...
def create_transaction(
    ...
    # update payment fee amount
    if (
        hasattr(gateway_response, "raw_response")
        and gateway_response.raw_response is not None
    ):
        if gateway_response.raw_response.get("object") == "payment_intent":
            charge_data = gateway_response.raw_response.get("charges").get("data")
            if len(charge_data) > 0:
                stripe_txn_id = charge_data[0].get("balance_transaction")
                company_stripe_account_id = payment_information.data.get(
                    "stripe_account_id"
                )
                stripe_txn = retrieve_txn(
                    api_key=STRIPE_SECRET_API_KEY,
                    company_stripe_account_id=company_stripe_account_id,
                    txn_id=stripe_txn_id,
                )

                if fee_details := stripe_txn.get("fee_details"):
                    for fee in fee_details:
                        if fee.get("type") == "stripe_fee":
                            payment.stripe_fee = Decimal(
                                float(fee.get("amount") * 0.01)
                            )
                        elif fee.get("type") == "application_fee":
                            payment.drural_fee = Decimal(
                                float(fee.get("amount") * 0.01)
                            )
...
def fetch_customer_id(user: User, gateway: str, stripe_account_id: str = None):
    """Retrieve users customer_id stored for desired gateway."""
    if stripe_account_id:
        return user.get_value_from_private_metadata(key=stripe_account_id)
    else:
...
def store_customer_id(
    user: User, gateway: str, customer_id: str, stripe_account_id: str = None
):
    """Store customer_id in users private meta for desired gateway."""
    meta_key = prepare_key_for_gateway_customer_id(gateway)
    if stripe_account_id:
        user.store_value_in_private_metadata(items={stripe_account_id: customer_id})
    else:
...
def price_from_minor_unit(value: str, currency: str):
    ...
    (value: 1000, currency: EUR) will be converted to 10.00
    """
...
def sum_totoal_refunded(qs):
    amount = sum([order.total - order.captured_amount for order in qs], Decimal("0.00"))
    return Money(amount, currency=DEFAULT_CURRENCY)


def search_customer(qs, value):
    qs = qs.filter(
        Q(order__user_email__trigram_similar=value)
        | Q(order__user__email__trigram_similar=value)
        | Q(order__user__first_name__trigram_similar=value)
        | Q(order__user__last_name__trigram_similar=value)
    )
    return qs
