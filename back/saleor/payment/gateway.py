...
def refund(
    ...
    if hasattr(
        payment.order.lines.first().variant.product.company, "stripe_credentials"
    ):
        stripe_account_id = (
            payment.order.lines.first().variant.product.company.stripe_credentials.account_id
        )
    else:
    ...
    payment_data = create_payment_information(
        ...
        payment_token=token,
        amount=amount,
        additional_data={"stripe_account_id": stripe_account_id},
    )
