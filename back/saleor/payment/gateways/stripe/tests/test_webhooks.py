...
def test_handle_successful_payment_intent_for_checkout(
    ...
    handle_successful_payment_intent(payment_intent, plugin.config, channel_EUR.slug)
...
def test_handle_successful_payment_intent_with_future_usage(
    ...
    mocked_payment_method_modify.assert_called_once_with(
        "payment_method_id",
        api_key="secret_key",
        metadata={"channel": channel_EUR.slug},
    )
...
def test_handle_successful_payment_intent_for_order(
    wrapped_checkout_complete, payment_stripe_for_order, stripe_plugin, channel_EUR
):
...
def test_handle_authorized_payment_intent_for_checkout(
    ...
    handle_authorized_payment_intent(payment_intent, plugin.config, channel_EUR.slug)
...
def test_handle_processing_payment_intent_for_order(
    ...
    handle_processing_payment_intent(payment_intent, plugin.config, channel_EUR.slug)
...
def test_handle_failed_payment_intent_for_checkout(
    stripe_plugin, payment_stripe_for_checkout, channel_EUR
):
    payment = payment_stripe_for_checkout
    payment.transactions.create(
        is_success=True,
    ...
    handle_failed_payment_intent(payment_intent, plugin.config, channel_EUR.slug)
...
def test_handle_failed_payment_intent_for_order(
    stripe_plugin, payment_stripe_for_order, channel_EUR
):
    payment = payment_stripe_for_order
    payment.transactions.create(
        is_success=True,
...
def test_handle_fully_refund(stripe_plugin, payment_stripe_for_order, channel_EUR):
    payment = payment_stripe_for_order
    ...
    handle_refund(charge, plugin.config, channel_EUR.slug)
...
def test_handle_partial_refund(stripe_plugin, payment_stripe_for_order, channel_EUR):
    payment = payment_stripe_for_order
    payment.captured_amount = payment.total
    payment.save()
    payment.transactions.create(
...
def test_handle_webhook_events(
    mocked_webhook_event, webhook_type, fun_to_mock, stripe_plugin, rf, channel_EUR
):
    dummy_payload = {
    ...
    with patch(f"saleor.payment.gateways.stripe.webhooks.{fun_to_mock}") as mocked_fun:
        ...
        mocked_fun.assert_called_once_with(
            event.data.object, plugin.config, channel_EUR.slug
        )
