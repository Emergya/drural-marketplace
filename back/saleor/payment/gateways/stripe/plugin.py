...
class StripeGatewayPlugin(BasePlugin):
    ...
    DEFAULT_CONFIGURATION = [
        {"name": "public_api_key", "value": settings.STRIPE_PUBLIC_API_KEY},
        {"name": "secret_api_key", "value": settings.STRIPE_SECRET_API_KEY},
        {"name": "automatic_payment_capture", "value": True},
        {"name": "supported_currencies", "value": settings.STRIPE_SUPPORTED_CURRENCIES},
        {"name": "webhook_endpoint_id", "value": settings.STRIPE_WEBHOOK_ENDPOINT_ID},
        {"name": "webhook_secret_key", "value": settings.STRIPE_WEBHOOK_SECRET_KEY},
    ]
    ...
    def process_payment(
        ...
        stripe_account_id = payment_information.data["stripe_account_id"]
        customer = None
        total_amount = round(payment_information.amount * 100)
        drural_amount = round(
            SiteSettings.objects.first().commission_rate * total_amount
        )
        ...
        intent, error = create_payment_intent(
            ...
            drural_fee_amount=drural_amount,
            company_stripe_account_id=stripe_account_id,
    ...
    def confirm_payment(
        ...
        if payment_intent_id:
            payment_intent, error = retrieve_payment_intent(
                api_key, payment_intent_id, stripe_account_id
            )
    ...
    def refund_payment(
        ...
        refund, error = refund_payment_intent(
            ...
            account_id=payment_information.data["stripe_account_id"],
        )
