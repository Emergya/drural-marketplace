...
@pytest.mark.parametrize(
    ...
    [(Decimal("15.00"), True), (Decimal("15.0"), False)],
)
def test_get_price(
    product_type,
    category,
    ...
    product = models.Product.objects.create(
        product_type=product_type, category=category, company=company
    )
    ...
    price = variant.get_price(
        product, [], channel_EUR, channel_listing, product.categories.all(), discounts
    )
    assert price.amount == expected_price


def test_product_get_price_do_not_charge_taxes(
    product_type, category, company, discount_info, channel_EUR
):
    product = models.Product.objects.create(
...
def test_costs_get_margin_for_variant_channel_listing(
    variant, price, cost, channel_EUR
):
