...
def create_logo(image_name="product2"):
    img_data = BytesIO()
    image = Image.new("RGB", size=(1, 1), color=(255, 0, 0, 0))
    image.save(img_data, format="JPEG")
    image = SimpleUploadedFile(image_name + ".svg", img_data.getvalue(), "image/png")
    return image, image_name


def create_banner(image_name="banner2"):
    img_data = BytesIO()
    image = Image.new("RGB", size=(1440, 480), color=(255, 0, 0, 0))
    image.save(img_data, format="JPEG")
    image = SimpleUploadedFile(image_name + ".jpg", img_data.getvalue(), "image/png")
    return image, image_name
