...
from ..payment.gateway import _validate_refund_amount
...
@traced_atomic_transaction()
def mark_order_as_paid_with_amount(
    order: "Order",
    request_user: "User",
    app: Optional["App"],
    manager: "PluginsManager",
    ...
    """Mark order as paid with amounts.

    Allows to create a payment for an order without actually performing any
    payment by the gateway.
    """
    amount_total = order.total.gross.amount

    if amount and amount_total and amount > amount_total:
        raise ValidationError(
            {
                "amount": ValidationError(
                    "Amount should not be greater than order total.",
    ...
    if float(amount) < float(amount_total):
