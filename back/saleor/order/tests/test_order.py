...
def test_add_variant_to_draft_order_adds_line_for_new_variant_with_tax(
    ...
    lines_before = order.lines.count()
    settings.LANGUAGE_CODE = "fr"
    unit_price = TaxedMoney(net=Money(8, "EUR"), gross=Money(10, "EUR"))
    total_price = TaxedMoney(net=Money("30.34", "EUR"), gross=Money("36.49", "EUR"))
    manager = Mock(
...
def test_add_variant_to_draft_order_adds_line_for_variant_with_price_0(
    ...
    variant_channel_listing.price = Money(0, "EUR")
...
def test_order_queryset_confirmed(draft_order, channel_EUR):
    other_orders = [
        Order.objects.create(status=OrderStatus.UNFULFILLED, channel=channel_EUR),
        Order.objects.create(
            status=OrderStatus.PARTIALLY_FULFILLED, channel=channel_EUR
        ),
        Order.objects.create(status=OrderStatus.FULFILLED, channel=channel_EUR),
        Order.objects.create(status=OrderStatus.CANCELED, channel=channel_EUR),
    ]
...
def test_order_queryset_drafts(draft_order, channel_EUR):
    other_orders = [
        Order.objects.create(status=OrderStatus.UNFULFILLED, channel=channel_EUR),
        Order.objects.create(
            status=OrderStatus.PARTIALLY_FULFILLED, channel=channel_EUR
...
def test_order_queryset_to_ship(settings, channel_EUR):
    total = TaxedMoney(net=Money(10, "EUR"), gross=Money(15, "EUR"))
    orders_to_ship = [
        Order.objects.create(
            status=OrderStatus.UNFULFILLED, total=total, channel=channel_EUR
        ),
        Order.objects.create(
            status=OrderStatus.PARTIALLY_FULFILLED, total=total, channel=channel_EUR
        ),
    ]
    ...
    orders_not_to_ship = [
        Order.objects.create(
            status=OrderStatus.DRAFT, total=total, channel=channel_EUR
        ),
        Order.objects.create(
            status=OrderStatus.UNFULFILLED, total=total, channel=channel_EUR
        ),
        ...
        Order.objects.create(
            status=OrderStatus.FULFILLED, total=total, channel=channel_EUR
        ),
        Order.objects.create(
            status=OrderStatus.CANCELED, total=total, channel=channel_EUR
        ),
    ]
...
def test_queryset_ready_to_capture(channel_EUR):
    total = TaxedMoney(net=Money(10, "EUR"), gross=Money(15, "EUR"))
    ...
    Order.objects.create(status=OrderStatus.DRAFT, total=total, channel=channel_EUR)
    Order.objects.create(
        status=OrderStatus.UNFULFILLED, total=total, channel=channel_EUR
    )
    Order.objects.create(status=OrderStatus.CANCELED, total=total, channel=channel_EUR)
...
def test_update_order_prices(
    ...
    price_1 = variant_1.get_price(
        ...
        [], 
        channel, 
        variant_channel_listing_1, 
        product_1.categories.all(), 
        None
    )
    ...
    price_2 = variant_2.get_price(
        ...
        [], 
        channel, 
        variant_channel_listing_2, 
        product_2.categories.all(),
        None
    )
...
def test_value_voucher_order_discount(
    ...
    order = Mock(
        get_subtotal=Mock(return_value=subtotal), voucher=voucher, channel=channel_EUR
    )
...
def test_shipping_voucher_order_discount(
    shipping_cost, discount_value, discount_type, expected_value, channel_EUR
):
    voucher = Voucher.objects.create(
        code="unique",
    ...
    shipping_total = TaxedMoney(
        gross=Money(shipping_cost, "EUR"), net=Money(shipping_cost, "EUR")
    )
    order = Mock(
...
def test_product_voucher_checkout_discount_raises_not_applicable(
    order_with_lines, product_with_images, channel_EUR
):
...
# @pytest.mark.skipif((not FEATURE_SALES or not FEATURE_VOUCHERS) , reason="discounts are not being used.")
