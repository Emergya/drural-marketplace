...
def test_match_orders_with_new_user(customer_user, channel_EUR):
...
def test_match_draft_order_with_new_user(customer_user, channel_EUR):
    address = customer_user.default_billing_address.get_copy()
    order = Order.objects.create(
        billing_address=address,
        user=None,
...
def test_update_taxes_for_order_lines(order_with_lines):
    ...
    unit_price = TaxedMoney(net=Money("10.23", "EUR"), gross=Money("15.80", "EUR"))
