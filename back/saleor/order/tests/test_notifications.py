...
def test_get_order_line_payload(order_line):
    ...
    assert payload == {
        ...
        "unit_price_net_amount": convert_to_current_currency(order_line.unit_price_net_amount),
        "unit_price_gross_amount": convert_to_current_currency(order_line.unit_price_gross_amount),
        "unit_tax_amount": convert_to_current_currency(unit_tax_amount),
        "total_gross_amount": convert_to_current_currency(total_gross.amount),
        "total_net_amount": convert_to_current_currency(total_net.amount),
        "total_tax_amount": convert_to_current_currency(total_tax.amount),
        "tax_rate": convert_to_current_currency(order_line.tax_rate),
        "is_digital": order_line.is_digital,
        "digital_url": "",
        "unit_discount_amount": convert_to_current_currency(order_line.unit_discount_amount),
        "unit_discount_reason": order_line.unit_discount_reason,
        "unit_discount_type": order_line.unit_discount_type,
        "unit_discount_value": convert_to_current_currency(order_line.unit_discount_value),
    }
...
def test_get_default_order_payload(order_line):
    ...
    exper_payload = {
        # "discounts": [
        #     {
        #         "amount_value": convert_to_current_currency(Decimal("20.000")),
        #         "name": None,
        #         "reason": "Discount reason",
        #         "translated_name": None,
        #         "type": "manual",
        #         "value": convert_to_current_currency(Decimal("20.000")),
        #         "value_type": "fixed",
        #     }
        # ],
        "discounts": [],
...
def test_send_email_payment_confirmation(mocked_notify, site_settings, payment_dummy):
    ...
    expected_payload = {
        ...
        "payment": {
            ...
            "total": convert_to_current_currency(payment_dummy.total),
            "captured_amount": convert_to_current_currency(payment_dummy.captured_amount),
            "currency": current_currency(),
        },
        "site_name": "mirumee.com",
        "domain": "mirumee.com",
...
@mock.patch("saleor.plugins.manager.PluginsManager.notify")
def test_send_email_order_confirmation(mocked_notify, order, order_line, site_settings):
    manager = get_plugins_manager()
    redirect_url = "https://www.example.com"

    order.lines.first().variant.product.company.managers.clear()
    order.save()
    order.user.language_code = "es"
    order.user.save()
...
def test_send_confirmation_emails_without_addresses_for_order(
    ...
    # staff_expected_payload = {
    #     "order": user_expected_payload["order"],
