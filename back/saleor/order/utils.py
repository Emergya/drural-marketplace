...
def sum_order_fees(qs, currency_code):
    zero = Money(0, currency=currency_code)
    taxed_zero = TaxedMoney(zero, zero)
    return sum([order.total for order in qs], taxed_zero)
...
def get_discounted_lines(lines, voucher):
    ...
    if discounted_products or discounted_collections or discounted_categories:
        for line in lines:
            ...
            line_categories = set(line.variant.product.categories.all())
            line_collections = set(line.variant.product.collections.all())
            if line.variant and (
                line_product in discounted_products
                or line_categories in discounted_categories
...
def get_custom_email_title_by_order(order):
    price = (
        order.total_gross_amount
        if order.display_gross_prices
        else order.total_net_amount
    )
    title = order.lines.first().variant.product.purchase_email.title
    title = title.replace("{{user_name}}", order.user.first_name)
    title = title.replace("{{product_name}}", order.lines.first().variant.product.name)
    title = title.replace("{{price}}", str(price))
    title = title.replace("{{order.currency}}", order.currency)
    if order.billing_address:
        billing_address = (
            order.billing_address.street_address_1 + ", " + order.billing_address.city
        )
        title = title.replace("{{order.billing_address}}", billing_address)

    if order.shipping_address:
        shipping_address = (
            order.shipping_address.street_address_1 + ", " + order.shipping_address.city
        )
        title = title.replace("{{order.shipping_address}}", shipping_address)

    if order.booking:
        title = title.replace(
            "{{booking.reference}}", str(order.booking.booking_reference)
        )
        title = title.replace(
            "{{booking.bookable_resource.name}}", order.booking.bookable_resource.name
        )
        title = title.replace(
            "{{booking.start_date}}",
            order.booking.start_date.strftime("%d/%m/%y %H:%M"),
        )
        title = title.replace(
            "{{product_duration}}",
            str(order.lines.first().variant.product.duration),
        )
    return title


def get_custom_email_content_by_order(order):
    price = (
        order.total_gross_amount
        if order.display_gross_prices
        else order.total_net_amount
    )
    content = order.lines.first().variant.product.purchase_email.content
    content = content.replace("{{user_name}}", order.user.first_name)
    content = content.replace(
        "{{product_name}}", order.lines.first().variant.product.name
    )
    content = content.replace("{{price}}", str(price))
    content = content.replace("{{order.currency}}", order.currency)

    if order.billing_address:
        billing_address = (
            order.billing_address.street_address_1 + ", " + order.billing_address.city
        )
        content = content.replace("{{order.billing_address}}", billing_address)

    if order.shipping_address:
        shipping_address = (
            order.shipping_address.street_address_1 + ", " + order.shipping_address.city
        )
        content = content.replace("{{order.shipping_address}}", shipping_address)
