...
def test_applicable_shipping_methods_price(
    ...
    ShippingMethodChannelListing.objects.create(
        currency=other_channel_EUR.currency_code,
        minimum_order_price_amount=min_price,
        maximum_order_price_amount=max_price,
        shipping_method=method,
        channel=other_channel_EUR,
    )
...
def test_applicable_shipping_methods_weight(
    weight, min_weight, max_weight, shipping_included, shipping_zone, channel_EUR
):
    method = shipping_zone.shipping_methods.create(
    ...
    ShippingMethodChannelListing.objects.create(
        shipping_method=method, channel=channel_EUR, currency=channel_EUR.currency_code
    )
...
def test_applicable_shipping_methods(shipping_zone, channel_EUR):
    price_method = shipping_zone.shipping_methods.create(
        type=ShippingMethodType.PRICE_BASED,
    )
    weight_method = shipping_zone.shipping_methods.create(
...
def test_applicable_shipping_methods_with_excluded_products(
    shipping_zone, channel_EUR, product, product_with_single_variant
):
...
def test_applicable_shipping_methods_not_in_channel(shipping_zone, channel_EUR):
    price_method = shipping_zone.shipping_methods.create(
        type=ShippingMethodType.PRICE_BASED,
    )
    weight_method = shipping_zone.shipping_methods.create(
...
def test_use_default_shipping_zone(shipping_zone, channel_EUR):
    ...
    default_zone.channels.add(channel_EUR)
...
def test_applicable_shipping_methods_price_rate_use_proper_channel(
    shipping_zone, channel_EUR, other_channel_EUR
):
    # given
    ...
    ShippingMethodChannelListing.objects.bulk_create(
        [
            # price_method_1
            ShippingMethodChannelListing(
                minimum_order_price=Money("10.0", "EUR"),
                maximum_order_price=Money("100.0", "EUR"),
            ...
            # price_method_2
            ShippingMethodChannelListing(
                minimum_order_price=Money("4.0", "EUR"),
                maximum_order_price=Money("10.0", "EUR"),
            ...
            ShippingMethodChannelListing(
                minimum_order_price=Money("1.0", "EUR"),
                maximum_order_price=Money("4.0", "EUR"),
