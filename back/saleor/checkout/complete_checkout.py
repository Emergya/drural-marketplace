...
def _create_order(
    *,
    ...
    checkout = checkout_info.checkout
    checkout_line = checkout.lines.select_related("booking").first()
    booking = checkout_line.booking  # type: ignore
    ...
    if booking:
        # confirm is called only when product is bookable.
        booking.confirm()  # type: ignore
    ...
    if not any([line.variant.product.skip_stock for line in order_lines]):
        country_code = checkout_info.get_country()
...
@traced_atomic_transaction()
def _create_order_from_no_billable_booking(
    *,
    order_data: dict,
    user: User,
    app: Optional["App"],
    ...
    """Create an order from no billable booking.

    Current user's language is saved in the order so we can later determine
    which language to use when sending email.
    """
    order_line = order_data.pop("line")
    booking = order_data.pop("booking")
    ...
    # Save order_line related to order
    order_line.order_id = order.pk
    order_line.save()
    ...
    # Send the order confirmation email
    transaction.on_commit(lambda: send_order_confirmation(order, "", manager))
...
def _process_payment(
    ...
    try:
        payment_data["stripe_account_id"] = order_data["lines"][
            0
        ].variant.product.company.stripe_credentials.account_id
...
def complete_checkout(
    ...
    if payment and user.is_authenticated:
        stripe_account_id = order_data["lines"][
            0
        ].variant.product.company.stripe_credentials.account_id
        customer_id = fetch_customer_id(
            user=user, gateway=payment.gateway, stripe_account_id=stripe_account_id
        )
    ...
    if txn.customer_id and user.is_authenticated:
        store_customer_id(
            user, payment.gateway, txn.customer_id, stripe_account_id
        )  # type: ignore
...
def complete_checkout_paying_outside_platform(
    manager: "PluginsManager",
    checkout_info: "CheckoutInfo",
    lines: Iterable["CheckoutLineInfo"],
    discounts,
...
def complete_booking_no_billable(
    manager: "PluginsManager",
    booking,
    user,
    app,
    ...
    """Logic to convert no billable booking to order.
    ...
    channel_listing = product.channel_listings.select_related("channel").first()
    channel = channel_listing.channel
    currency = channel_listing.currency
    price_amount = channel_listing.discounted_price_amount
    ...
    line = OrderLine(
        ...
        variant=variant,
        currency=currency,
        unit_price_net_amount=price_amount,
        unit_price_gross_amount=price_amount,
        total_price_net_amount=price_amount,
        total_price_gross_amount=price_amount,
    )
    ...
    order_data = {
        ...
        "total_paid_amount": price_amount,
        "total_gross_amount": price_amount,
        "total_net_amount": price_amount,
    }

    try:
        _create_order_from_no_billable_booking(
            order_data=order_data,
            user=user,  # type: ignore
            app=app,
            manager=manager,
