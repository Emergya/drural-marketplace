...
def add_variants_to_checkout(
    ...
    # if no bookings create a a list of None
    if not bookings:
        bookings = [None for _ in variants]

    if remove_lines:
        checkout.lines.all().delete()
        items = []
        for variant, quantity, booking in zip(variants, quantities, bookings):
            items.append(
                CheckoutLine(
                    checkout=checkout,
                    variant=variant,
        ...
        if items:
            CheckoutLine.objects.bulk_create(items)
        return checkout
