...
@pytest.fixture()
def anonymous_checkout(db, channel_EUR):
    return Checkout.objects.get_or_create(user=None, channel=channel_EUR)[0]
...
def test_adding_same_variant(checkout, product):
    ...
    subtotal = TaxedMoney(Money("30.00", "EUR"), Money("30.00", "EUR"))
...
def test_get_prices_of_discounted_specific_product_only_product(
    ...
    excepted_value = [
        line.variant.get_price(
            product, [], channel, variant_channel_listing, product.categories.all(), []
        )
        for item in range(line.quantity)
    ]
