...
def test_get_discount_for_checkout_value_voucher(
    ...
    checkout = Mock(spec=checkout_with_items, channel=channel_EUR)
    subtotal = TaxedMoney(Money(total, "EUR"), Money(total, "EUR"))
    monkeypatch.setattr(
...
def test_get_discount_for_checkout_entire_order_voucher_not_applicable(
    ...
    checkout = Mock(spec=Checkout, channel=channel_EUR)
    subtotal = TaxedMoney(Money(total, "EUR"), Money(total, "EUR"))
    monkeypatch.setattr(
        "saleor.checkout.utils.calculations.checkout_subtotal",
        lambda manager, checkout_info, lines, address, discounts: subtotal,
...
def test_get_discount_for_checkout_specific_products_voucher_not_applicable(
    ...
    monkeypatch.setattr(
        ...
        lambda _: TaxedMoney(Money(0, "EUR"), Money(0, "EUR")),
    )
    monkeypatch.setattr(
        "saleor.discount.utils.calculations.checkout_subtotal",
        lambda manager, checkout_info, lines, address, discounts: TaxedMoney(
            Money(total, "EUR"), Money(total, "EUR")
        ),
    )
    monkeypatch.setattr(
        "saleor.checkout.utils.calculations.checkout_subtotal",
    ...
    checkout = Mock(quantity=total_quantity, spec=Checkout, channel=channel_EUR)
    checkout_info = CheckoutInfo(
        checkout=checkout,
        shipping_method=None,
        shipping_address=None,
...
def test_get_discount_for_checkout_shipping_voucher(
    ...
    manager = get_plugins_manager()
    subtotal = TaxedMoney(Money(100, "EUR"), Money(100, "EUR"))
    monkeypatch.setattr(
        "saleor.checkout.utils.calculations.checkout_subtotal",
        lambda manager, checkout_info, lines, address, discounts: subtotal,
    )
...
def test_get_discount_for_checkout_shipping_voucher_all_countries(
    monkeypatch, channel_EUR, shipping_method
):
    subtotal = TaxedMoney(Money(100, "EUR"), Money(100, "EUR"))
    monkeypatch.setattr(
        "saleor.checkout.utils.calculations.checkout_subtotal",
    ...
    shipping_total = TaxedMoney(Money(10, "EUR"), Money(10, "EUR"))
    monkeypatch.setattr(
    ...
    assert discount == Money(5, "EUR")


def test_get_discount_for_checkout_shipping_voucher_limited_countries(
    monkeypatch, channel_EUR
):
    subtotal = TaxedMoney(net=Money(100, "EUR"), gross=Money(100, "EUR"))
    shipping_total = TaxedMoney(net=Money(10, "EUR"), gross=Money(10, "EUR"))
    monkeypatch.setattr(
        "saleor.discount.utils.calculations.checkout_subtotal",
        lambda manager, checkout_info, lines, address, discounts: subtotal,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (
            True,
            Mock(
                get_total=Mock(return_value=Money(10, "EUR")),
            ...
            None,
            None,
            TaxedMoney(Money(10, "EUR"), Money(10, "EUR")),
            10,
        ...
        (
            True,
            Mock(price=Money(10, "EUR")),
            10,
            DiscountValueType.FIXED,
            [],
            5,
            None,
            TaxedMoney(Money(2, "EUR"), Money(2, "EUR")),
            10,
        ...
        (
            ...
            5,
            10,
            TaxedMoney(Money(5, "EUR"), Money(5, "EUR")),
            9,
...
def test_recalculate_checkout_discount(
    checkout_with_voucher, voucher, voucher_translation_fr, settings, channel_EUR
):
    settings.LANGUAGE_CODE = "fr"
    voucher.channel_listings.filter(channel=channel_EUR).update(discount_value=10)
    ...
    assert checkout_with_voucher.discount == Money("10.00", "EUR")
...
def test_recalculate_checkout_discount_with_sale(
    ...
    assert checkout.discount == Money("3.00", "EUR")
    assert (
...
def test_recalculate_checkout_discount_voucher_not_applicable(
    checkout_with_voucher, voucher, channel_EUR
):
    checkout = checkout_with_voucher
    voucher.channel_listings.filter(channel=channel_EUR).update(min_spent_amount=100)
...
def test_recalculate_checkout_discount_free_shipping_subtotal_less_than_shipping(
    ...
    lines = fetch_checkout_lines(checkout)
    checkout_info = fetch_checkout_info(checkout, lines, [], manager)
    channel_listing = shipping_method.channel_listings.get(channel_id=channel_EUR.id)
...
def test_recalculate_checkout_discount_free_shipping_subtotal_bigger_than_shipping(
    ...
    lines = fetch_checkout_lines(checkout)
    checkout_info = fetch_checkout_info(checkout, lines, [], manager)
    channel_listing = shipping_method.channel_listings.get(channel=channel_EUR)
    channel_listing.price = (
        calculations.checkout_subtotal(
            manager=manager,
            checkout_info=checkout_info,
