...
def test_export_products_filter_is_published(
    ...
    export_products(
        user_export_file,
        {"filter": {"is_published": True, "channel": channel_EUR.slug}},
        export_info,
        file_type,
    )
    ...
    assert set(args[0].values_list("pk", flat=True)) == set(
        Product.objects.filter(
            channel_listings__is_published=True, channel_listings__channel=channel_EUR
...
def test_save_csv_file_in_export_file(user_export_file, tmpdir, media_root):
    ...
    file_mock = TemporaryUploadedFile(
        name=file_name, size=500, charset="utf8", content_type="application/CSV"
    )
