...
def test_allocate_stocks(order_line, stock, channel_EUR):
    stock.quantity = 100
    stock.save(update_fields=["quantity"])

    line_data = OrderLineData(line=order_line, variant=order_line.variant, quantity=50)

    allocate_stocks([line_data], COUNTRY_CODE, channel_EUR.slug)
...
def test_allocate_stocks_multiple_lines(order_line, order, product, stock, channel_EUR):
    stock.quantity = 100
    stock.save(update_fields=["quantity"])
    ...
    allocate_stocks([line_data_1, line_data_2], COUNTRY_CODE, channel_EUR.slug)
...
def test_allocate_stock_many_stocks(order_line, variant_with_many_stocks, channel_EUR):
...
def test_allocate_stock_partially_allocated_insufficient_stocks(
    order_line, order_line_with_allocation_in_many_stocks, channel_EUR
):
    allocated_line = order_line_with_allocation_in_many_stocks
    variant = allocated_line.variant
    stocks = variant.stocks.all()
...
def test_allocate_stocks_no_channel_shipping_zones(order_line, stock, channel_EUR):
...
def test_allocate_stock_insufficient_stocks(
    order_line, variant_with_many_stocks, channel_EUR
):
    variant = variant_with_many_stocks
    stocks = variant.stocks.all()
...
def test_allocate_stock_insufficient_stocks_for_multiple_lines(
    order_line, variant_with_many_stocks, product, channel_EUR
):
    variant = variant_with_many_stocks
    stocks = variant.stocks.all()
