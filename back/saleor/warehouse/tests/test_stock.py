...
def test_stocks_for_country(variant_with_many_stocks, channel_EUR):
    [stock1, stock2] = (
        Stock.objects.filter(product_variant=variant_with_many_stocks)
        .for_country_and_channel(COUNTRY_CODE, channel_EUR.slug)
        .order_by("pk")
        .all()
    )
...
def test_stock_for_country_does_not_exists(product, warehouse, channel_EUR):
    ...
    stock_qs = Stock.objects.for_country_and_channel(
        fake_country_code, channel_EUR.slug
    )
