...
@pytest.mark.parametrize(
    ...
    [(Country("ES"), "EUR"), (Country("US"), "USD"), (Country("GB"), "GBP")],
)
...
def test_create_channels(db):
    ...
    for _ in random_data.create_channels():
        pass
    assert Channel.objects.all().count() == 1
    assert Channel.objects.get(slug="main")
...
def test_create_fake_user(db):
    assert User.objects.all().count() == 0
    random_data.create_fake_user("password", "media")
    assert User.objects.all().count() == 1
...
def test_create_fake_users(db):
    for _ in random_data.create_users("password", placeholder_dir="media", how_many=5):
        pass
...
@pytest.mark.skip(reason="It does not work with all tests")
def test_create_fake_order(db, monkeypatch, media_root, warehouse):
    # Tests shouldn't depend on images present in placeholder folder
    image_mock = tempfile.NamedTemporaryFile(suffix=".jpg").name
    monkeypatch.setattr(
        "saleor.core.utils.random_data.get_image", Mock(return_value=image_mock)
    )
    for _ in random_data.create_channels():
        pass
    for _ in random_data.create_shipping_zones():
        pass
    for _ in random_data.create_users("password", "media", 3):
        pass
    for msg in random_data.create_page_type():
        pass
    for msg in random_data.create_pages():
        pass
    random_data.create_staffs("dRural.!1234")
    random_data.create_products_by_schema("/", False)
    for _ in random_data.create_orders(how_many=2):
        pass
...
def test_storages_set_s3_bucket_domain(storage, settings):
    settings.MINIO_MK_MEDIA_BUCKET_NAME = "media-bucket"
    settings.MINIO_MK_MEDIA_CUSTOM_DOMAIN = "media-bucket.example.org"
...
def test_storages_not_setting_s3_bucket_domain(storage, settings):
    settings.MINIO_MK_MEDIA_BUCKET_NAME = "media-bucket"
    settings.MINIO_MK_MEDIA_CUSTOM_DOMAIN = None
    storage = S3MediaStorage()
    assert storage.bucket_name == "media-bucket"
...
def test_placeholder(settings):
    ...
    assert (
        result
        == "http://dev.marketplace.drural.emergyalabs.com:7000/drural-media/"
        + settings.PLACEHOLDER_IMAGES[size]
    )
