...
from ....settings import FEATURE_VOUCHERS, FEATURE_SALES, FEATURE_WAREHOUSES
...
def test_query_public_meta_for_staff_as_app(app_api_client, admin_user):
    # given
    admin_user.store_value_in_metadata({PUBLIC_KEY: PUBLIC_VALUE})
    admin_user.save(update_fields=["metadata"])
    variables = {"id": graphene.Node.to_global_id("User", admin_user.pk)}
    ...
    # when
    response = app_api_client.post_graphql(QUERY_USER_PUBLIC_META, variables)
...
def test_query_public_meta_for_order_as_staff(
    staff_api_client, order_with_lines, customer_user, permission_manage_orders
):
    # given
    order_with_lines.user = customer_user
    order_with_lines.store_value_in_metadata({PUBLIC_KEY: PUBLIC_VALUE})
    order_with_lines.save(update_fields=["user", "metadata"])
...
def test_query_public_meta_for_order_as_app(
    app_api_client, order_with_lines, customer_user, permission_manage_orders
):
    # given
    order_with_lines.user = customer_user
    order_with_lines.store_value_in_metadata({PUBLIC_KEY: PUBLIC_VALUE})
...
def test_query_public_meta_for_collection_as_anonymous_user(
    api_client, published_collection, channel_EUR
):
    # given
...
def test_query_public_meta_for_collection_as_staff(
    staff_api_client, published_collection, permission_manage_products, channel_EUR
):
    # given
    collection = published_collection
    collection.store_value_in_metadata({PUBLIC_KEY: PUBLIC_VALUE})
...
def test_query_public_meta_for_collection_as_app(
    app_api_client, published_collection, permission_manage_products, channel_EUR
):
    # given
    collection = published_collection
    collection.store_value_in_metadata({PUBLIC_KEY: PUBLIC_VALUE})
...
def test_query_public_meta_for_product_variant_as_anonymous_user(
    api_client, variant, channel_EUR
):
    # given
...
def test_query_private_meta_for_staff_as_app(app_api_client, admin_user):
    # given
    admin_user.store_value_in_private_metadata({PRIVATE_KEY: PRIVATE_VALUE})
    admin_user.save(update_fields=["private_metadata"])
    variables = {"id": graphene.Node.to_global_id("User", admin_user.pk)}
    ...
    # when
    response = app_api_client.post_graphql(QUERY_USER_PRIVATE_META, variables)
...
def test_query_private_meta_for_order_as_staff(
    ...
    # given
    order_with_lines.user = customer_user
    order_with_lines.store_value_in_private_metadata({PRIVATE_KEY: PRIVATE_VALUE})
    order_with_lines.save(update_fields=["user", "private_metadata"])
    variables = {"id": graphene.Node.to_global_id("Order", order_with_lines.pk)}
