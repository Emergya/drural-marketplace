...
from ..wishlist.resolvers import resolve_wishlists_from_user
from ..wishlist.sorters import WishlistSortingInput
...
class User(CountableDjangoObjectType):
    ...
    is_seller = graphene.Boolean(
        source="is_seller", description="Determines if an user is seller"
    )
    wishlists = PrefetchingConnectionField(
        "saleor.graphql.wishlist.types.Wishlist",
        description="List of user's wishlists.",
        sort_by=WishlistSortingInput(description="Sort wishlists."),
    )
    ...
    def resolve_addresses(root: models.User, _info, **_kwargs):
        ...
        if (
            (
                requestor is _info.context.app
        ...
        if requestor.is_seller:
            # filter only orders from the company
            orders = models.Order.objects.filter(
                lines__variant__product__company__in=requestor.companies.all(),
                user=root,
            )
            # get ids all of shipping and billing addresses of the orders
            if orders.exists():
                shipping_ids = orders.values_list(
                    "shipping_address_id", flat=True
                ).distinct()
                billing_ids = orders.values_list(
                    "billing_address_id", flat=True
                ).distinct()
                return models.Address.objects.filter(
                    Q(id__in=shipping_ids) | Q(id__in=billing_ids)
                ).distinct()
            else:
                return []

    @staticmethod
    def resolve_default_billing_address(root: models.User, _info, **_kwargs):
        requestor = get_user_or_app_from_context(_info.context)
        if requestor == root or requestor.is_staff:
            return root.default_billing_address
        else:
            raise PermissionDenied()

    @staticmethod
    def resolve_default_shipping_address(root: models.User, _info, **_kwargs):
        requestor = get_user_or_app_from_context(_info.context)
        if requestor == root or requestor.is_staff:
            return root.default_shipping_address
        else:
            raise PermissionDenied()
    ...
    def resolve_checkout_tokens(root: models.User, info, channel=None, **_kwargs):
        ...
        def return_checkout_tokens(checkouts):
            ...
            for checkout in checkouts:
                ...
                for line in lines:
                    is_manager = (
                        line.variant.product.company in requester.companies.all()
                    )
                    if is_manager:
                        break
                if requester == checkout.user or requester.is_staff or is_manager:
    ...
    def resolve_events(root: models.User, info):
        requester = get_user_or_app_from_context(info.context)
        return CustomerEventsByUserLoader(info.context).load(
            tuple([root.id, requester])
        )
    ...
    def resolve_orders(root: models.User, info, **_kwargs):
        ...
        return (
            OrdersByUserLoader(info.context)
            .load(tuple([root.id, requester]))
    ...
    @staticmethod
    def resolve_wishlists(root: models.User, info, **_kwargs):
        return resolve_wishlists_from_user(root)
...
class UserStat(graphene.ObjectType):
    total = graphene.Int(description="Total users registered at that date")
    date = graphene.Date(description="Date when we mesure the concurrent users.")
