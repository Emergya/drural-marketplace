...
class AccountRegisterInput(graphene.InputObjectType):
    first_name = graphene.String(
        description="The email first name of the user.", required=True
    )
    last_name = graphene.String(
        description="The email last name of the user.", required=True
    )
...
class AccountInput(graphene.InputObjectType):
    ...
    is_onboard = graphene.Boolean(description="Is on board")
...
class AccountLocationPreferencesInput(graphene.InputObjectType):
    distance = graphene.Int(
        required=False, description="Maximum distance to search for services."
    )
    is_location_allowed = graphene.Boolean(
        required=True, description="Allow user location to be tracked."
    )
...
class SetAccountCategoriesPreferences(BaseMutation):
    user = graphene.Field(User, description="A user instance with new categories.")

    class Arguments:
        categories = graphene.List(
            graphene.ID,
            description="A list of categories IDs preferred by user.",
            required=True,
        )

    class Meta:
        description = "Updates user's categories preferences."
        error_type_class = AccountError
        error_type_field = "account_errors"
    ...
    def perform_mutation(cls, _root, info, **data):
        user = info.context.user
        category_global_ids = data.get("categories")
        categories_ids = [
            # Only return id
            from_global_id_or_error(
                category_id, only_type="Category", raise_error=True
            )[1]
            for category_id in category_global_ids
        ]
        user.categories.set(categories_ids)
        return SetAccountCategoriesPreferences(user=user)


class SetAccountLocationPreferences(ModelMutation):
    class Arguments:
        input = AccountLocationPreferencesInput(
            required=True, description="Fields required to set location preferences."
        )

    class Meta:
        description = "Updates user's location preferences."
        model = models.User
        error_type_class = AccountError
        error_type_field = "account_errors"
