...
class PasswordChange(BaseMutation):
    ...
    class Arguments:
        ...
        new_password_repeat = graphene.String(
            required=True, description="Repeat the password."
        )
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        if new_password != new_password_repeat:
            raise ValidationError(
                {
                    "new_password_repeat": ValidationError(
                        "Repeat password field does not match with the new password.",
                        code=AccountErrorCode.PASSWORD_REPEAT_NOT_MATCH,
                    )
                }
            )
