...
def can_manage_app(requestor: Union["User", "App"], app: "App") -> bool:
    ...
    return requestor.has_perms(permissions) and requestor == app.user
...
def company_by_event(event):
    company = None
    if event.type == CustomerEvents.PLACED_ORDER:
        company = event.order.lines.first().variant.product.company
    return company
