...
"""
AGENT_USERS_QUERY = """
    query User($company: ID!) {
        agentUsers(first: 100, company: $company) {
            edges {
                node {
                    id
                    firstName
...
def test_query_agent_users(
    ...
    response = staff_api_client.post_graphql(AGENT_USERS_QUERY, variables)
    content = get_graphql_content(response)

    data = content["data"]["agentUsers"]['edges']

    assert data[0]['node']['email'] == staff_users[1].email
    assert data[1]['node']['email'] == staff_users[2].email

    assert data[0]['node']['isSeller'] == True
    assert data[1]['node']['isSeller'] == True

AGENT_USERS_DETAILS_QUERY = """
    query User($id: ID, $email: String) {
        agentUserDetails(id: $id, email: $email) {
            id
            firstName
            lastName
            email
...
def test_query_agent_user_details_by_id(
    user_api_client,
    company,
    seller_user,
    permission_manage_users,
):
    user_api_client.user.companies.add(company)
    seller_user.companies.add(company)
    add_user_in_sellers_group(seller_user)
    add_user_in_sellers_group(user_api_client.user)

    user_api_client.user.user_permissions.add(permission_manage_users)

    variables = {
        "id": graphene.Node.to_global_id("User", seller_user.id)
        }
    response = user_api_client.post_graphql(AGENT_USERS_DETAILS_QUERY, variables)
    content = get_graphql_content(response)

    data = content["data"]["agentUserDetails"]
    assert data["id"] == graphene.Node.to_global_id("User", seller_user.id)
    assert data["firstName"] == seller_user.first_name
    assert data["lastName"] == seller_user.last_name
    assert data["email"] == seller_user.email
    assert data["isStaff"] == seller_user.is_staff


def test_query_agent_user_details_by_email(
    user_api_client,
    company,
    seller_user,
    permission_manage_users,
...
def test_query_agent_user_details_user_no_agent(
    user_api_client,
    seller_user,
    permission_manage_users,
):
...
def test_query_customer_user(
    ...
    avatar_mock = tempfile.NamedTemporaryFile(suffix=".jpg").name
...
def test_me_checkout_tokens_without_channel_param_inactive_channel(
    user_api_client, channel_EUR, checkouts_assigned_to_customer
):
    # given
    ...
    content = get_graphql_content(response)
    data = content["data"]["me"]
    assert str(checkouts[0].token) not in data["checkoutTokens"]
...
def test_me_checkout_tokens_with_channel(
    ...
    # when
    response = user_api_client.post_graphql(
        QUERY_ME_CHECKOUT_TOKENS, {"channel": channel_EUR.slug}
    )
...
def test_customer_register(mocked_notify, mocked_generator, api_client, channel_EUR):
    ...
    expected_payload = {
        ...
        "email_image": STATIC_URL + "welcome-image.png",
        "user_name": "Customer name",
        "domain": "mirumee.com",
        "site_name": "mirumee.com",
        "site_logo": STATIC_URL + "logo-horizontal-positive.png",
        "site_icon_logo": STATIC_URL + "logo-icon-mono-positive.png",
...
def test_customer_update_generates_event_when_changing_email_by_app(
    ...
    email_changed_event = account_events.CustomerEvent.objects.get()
    assert email_changed_event.type == account_events.CustomerEvents.EMAIL_ASSIGNED
    assert email_changed_event.user == staff_user
    assert email_changed_event.parameters == {"message": "mirumee@example.com"}
...
def test_account_request_deletion(
    mocked_notify, mocked_token, user_api_client, channel_EUR
):
    mocked_token.return_value = "token"
    user = user_api_client.user
    redirect_url = "https://www.example.com"
    variables = {"redirectUrl": redirect_url, "channel": channel_EUR.slug}
    response = user_api_client.post_graphql(
...
def test_account_request_deletion_token_validation(
    mocked_notify, user_api_client, channel_EUR
):
    user = user_api_client.user
...
def test_account_request_deletion_all_storefront_hosts_allowed(
    mocked_notify, user_api_client, settings, channel_EUR
):
    user = user_api_client.user
...
def test_customer_delete_by_app(
    ...
    assert kwargs["staff_user"] == staff_user
    delete_versatile_image_mock.assert_called_once_with(customer_user.avatar)
...
def test_staff_delete(
    staff_api_client, permission_manage_staff, permission_manage_users
):
    ...
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_staff, permission_manage_users]
    )
    content = get_graphql_content(response)
...
def test_set_password(user_api_client, customer_user):
    ...
    password_resent_event = account_events.CustomerEvent.objects.first()
...
def test_password_change_incorrect_old_password(user_api_client):
    customer_user = user_api_client.user
    variables = {"oldPassword": "incorrect", "newPassword": "", "newPasswordRepeat": ""}
    response = user_api_client.post_graphql(CHANGE_PASSWORD_MUTATION, variables)
    content = get_graphql_content(response)
    data = content["data"]["passwordChange"]
    customer_user.refresh_from_db()
...
def test_account_reset_password(
    mocked_notify, user_api_client, customer_user, channel_EUR
):
    redirect_url = "https://www.example.com"
    variables = {
        "email": customer_user.email,
...
def test_account_confirmation(
    match_orders_with_new_user_mock, api_client, customer_user, channel_EUR
):
...
def test_account_confirmation_invalid_user(
    match_orders_with_new_user_mock, user_api_client, customer_user, channel_EUR
):
    variables = {
...
def test_request_password_reset_email_for_staff(
    mocked_notify, staff_api_client, channel_EUR
):
    redirect_url = "https://www.example.com"
...
def test_account_reset_password_all_storefront_hosts_allowed(
    mocked_notify, user_api_client, customer_user, settings, channel_EUR
):
    settings.ALLOWED_CLIENT_HOSTS = ["*"]
    redirect_url = "https://www.test.com"
    variables = {
...
def test_user_avatar_update_mutation(monkeypatch, staff_api_client, media_root):
    ...
    assert data["user"]["avatar"]["url"].startswith(
        "http://testserver/media/user-avatars/avatar_"
    )
...
@pytest.mark.skip(reason="Every use can now delete his own image now")
...
def test_query_customers_with_filter_placed_orders(
    ...
    Order.objects.create(user=customer_user, channel=channel_EUR)
    second_customer = User.objects.create(email="second_example@example.com")
    with freeze_time("2012-01-14 11:00:00"):
        Order.objects.create(user=second_customer, channel=channel_EUR)
...
def test_query_customers_with_filter_placed_orders_(
    ...
    Order.objects.bulk_create(
        [
            Order(user=customer_user, token=str(uuid.uuid4()), channel=channel_EUR),
            Order(user=customer_user, token=str(uuid.uuid4()), channel=channel_EUR),
            Order(user=customer_user, token=str(uuid.uuid4()), channel=channel_EUR),
        ]
    )
...
def test_query_customers_with_sort(
    customer_sort, result_order, staff_api_client, permission_manage_users, channel_EUR
):
    ...
    Order.objects.create(
        user=User.objects.get(email="zordon01@example.com"), channel=channel_EUR
    )
...
@pytest.mark.skip(reason="Staff can be managed by apps")
...
def test_query_staff_members_with_sort(
    customer_sort, result_order, staff_api_client, permission_manage_staff, channel_EUR
):
    User.objects.bulk_create(
        [
            User(
...
def test_addresses_query_as_anonymous_user(api_client):
    variables = {"id": ""}
    response = api_client.post_graphql(ADDRESSES_USER_QUERY, variables)
    assert_no_permission(response)


def test_addresses_query_user_with_addresses(user_api_client, customer_user):
    addresses = customer_user.addresses.all()
    response = user_api_client.post_graphql(ADDRESSES_QUERY)
    content = get_graphql_content(response)
    data = content["data"]["me"]
    assert len(data["addresses"]) == len(addresses)


def test_addresses_query_staff(
    staff_api_client, customer_user, permission_manage_users
):
    variables = {"id": graphene.Node.to_global_id("User", customer_user.id)}

    addresses = customer_user.addresses.all()
    staff_api_client.user.user_permissions.add(permission_manage_users)
    response = staff_api_client.post_graphql(ADDRESSES_USER_QUERY, variables)
    content = get_graphql_content(response)
    data = content["data"]["user"]
    assert len(data["addresses"]) == len(addresses)
    data["addresses"][0]["streetAddress1"] == addresses.get().street_address_1
...
def test_addresses_query_as_app_with_permission(
    app_api_client, permission_manage_users, customer_user
):
    variables = {"id": graphene.Node.to_global_id("User", customer_user.id)}
    addresses = customer_user.addresses.all()
    response = app_api_client.post_graphql(
        ADDRESSES_USER_QUERY, variables, permissions=[permission_manage_users]
    )
    content = get_graphql_content(response)
    data = content["data"]["user"]
    assert len(data["addresses"]) == len(addresses)
...
def test_addresses_query_as_seller(
    user_api_client, customer_user, permission_manage_users
):
    variables = {"id": graphene.Node.to_global_id("User", customer_user.id)}
    addresses = customer_user.addresses.all()
    ...
    data = content["data"]["user"]
    assert len(data["addresses"]) == len(addresses)
    assert data["addresses"][0]["streetAddress1"] == addresses.get().street_address_1
...
def test_request_email_change(user_api_client, customer_user, channel_EUR):
    variables = {
        "password": "password",
...
def test_email_update(user_api_client, customer_user, channel_EUR):
    new_email = "new_email@example.com"
    payload = {
    ...
    variables = {"token": token, "channel": channel_EUR.slug}
