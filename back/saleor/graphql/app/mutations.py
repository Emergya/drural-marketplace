...
from ...graphql.account.types import User as UserType
...
class AppInput(graphene.InputObjectType):
    ...
    user_id = graphene.ID(description="ID of user asociated with the app.")
...
class AppTokenCreate(ModelMutation):
    ...
    def clean_input(cls, info, instance, data):
        ...
        if (
            not requestor_is_superuser(requestor)
            and not requestor_is_staff_member_or_app(requestor)
            and not can_manage_app(requestor, app)
        ):
...
class AppCreate(ModelMutation):
    ...
    def clean_input(cls, info, instance, data, input_cls=None):
        ...
        _, user_pk = from_global_id_or_error(user_id, UserType)
        user = User.objects.filter(pk=user_pk).first()
        if not user_id:
            raise ValidationError(
                {
                    "user_id": ValidationError(
                        "You have to link the app with a user",
        ...
        if (
            ...
            not user.is_seller
            and not requestor_is_superuser(user)
            and not requestor_is_staff_member_or_app(user)
        ):
            raise ValidationError(
                {
                    "user_id": ValidationError(
                        "You cannot create an app for this user",
        ...
        if "permissions" in cleaned_input:
            ...
            ensure_can_manage_permissions(user, permissions)
        return cleaned_input
...
class AppUpdate(ModelMutation):
    ...
    def clean_input(cls, info, instance, data, input_cls=None):
        ...
        if (
            not requestor_is_superuser(requestor)
            and not requestor_is_staff_member_or_app(requestor)
            and not can_manage_app(requestor, instance)
        ):
            msg = "You can't manage this app."
            code = AppErrorCode.OUT_OF_SCOPE_APP.value
            raise ValidationError({"id": ValidationError(msg, code=code)})
...
class AppActivate(ModelMutation):
    ...
    def perform_mutation(cls, root, info, **data):
        ...
        if not requestor_is_superuser(requestor) and not can_manage_app(requestor, app):
            msg = "You can't activate this app."
            code = AppErrorCode.OUT_OF_SCOPE_APP.value
            raise ValidationError({"id": ValidationError(msg, code=code)})
