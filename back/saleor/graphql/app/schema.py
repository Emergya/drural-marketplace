...
from ...graphql.utils import get_user_or_app_from_context, requestor_is_superuser
...
class AppQueries(graphene.ObjectType):
    ...
    def resolve_apps(self, info, **kwargs):
        ...
        if requestor_is_superuser(requestor) or requestor_is_staff_member_or_app(
            requestor
        ):
            return resolve_apps(info, **kwargs)
        else:
            return resolve_apps_by_user(info, requestor, **kwargs)
