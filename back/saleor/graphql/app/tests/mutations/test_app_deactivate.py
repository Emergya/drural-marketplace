...
def test_deactivate_app_by_app(app, app_api_client, permission_manage_apps, staff_user):
    # given
...
def test_app_has_more_permission_than_app_requestor(
    seller_app_api_client, permission_manage_orders, permission_manage_apps, staff_user
):
    # given
    app = App.objects.create(name="Sample app objects", is_active=True, user=staff_user)
    app.permissions.add(permission_manage_orders)
    ...
    # when
    response = seller_app_api_client.post_graphql(
        query, variables=variables, permissions=(permission_manage_apps,)
    )
    ...
    assert (
        content["data"]["appDeactivate"]["errors"][0]["code"]
        == AppErrorCode.OUT_OF_SCOPE_APP.name
    )
