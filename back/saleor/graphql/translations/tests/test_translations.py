...
from ....settings import FEATURE_VOUCHERS, FEATURE_SALES, FEATURE_SHIPPING
...
def test_product_translation(user_api_client, product, channel_EUR):
    ...
    response = user_api_client.post_graphql(
        query, {"productId": product_id, "channel": channel_EUR.slug}
    )
...
def test_product_translation_without_description(user_api_client, product, channel_EUR):
...
def test_product_translation_with_app(app_api_client, product, channel_EUR):
    product.translations.create(language_code="pl", name="Produkt")
...
def test_product_variant_translation(user_api_client, variant, channel_EUR):
    ...
    response = user_api_client.post_graphql(
        query, {"productVariantId": product_variant_id, "channel": channel_EUR.slug}
    )
    data = get_graphql_content(response)["data"]
...
def test_collection_translation(user_api_client, published_collection, channel_EUR):
    description = dummy_editorjs("test desription")
    ...
    variables = {"collectionId": collection_id, "channel": channel_EUR.slug}
...
def test_product_no_translation(user_api_client, product, channel_EUR):
    query = """
    query productById($productId: ID!, $channel: String) {
        product(id: $productId, channel: $channel) {
            translation(languageCode: PL) {
...
def test_product_variant_no_translation(user_api_client, variant, channel_EUR):
    query = """
    query productVariantById($productVariantId: ID!, $channel: String) {
        productVariant(id: $productVariantId, channel: $channel) {
            translation(languageCode: PL) {
...
def test_collection_no_translation(user_api_client, published_collection, channel_EUR):
    query = """
    query collectionById($collectionId: ID!, $channel: String) {
        collection(id: $collectionId, channel: $channel) {
            translation(languageCode: PL) {
...
def test_product_create_translation_with_app(
    ...
    assert data['errors'][0]['extensions']['exception']['code'] == 'PermissionDenied'
...
def test_product_and_attribute_translation(user_api_client, product, channel_EUR):
    description = dummy_editorjs("test desription")
    product.translations.create(
        language_code="pl", name="Produkt", description=description
    )
