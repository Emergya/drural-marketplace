...
from ...core.permissions import CheckoutPermissions, SitePermissions
...
from ..product.types import Product, ProductVariant, Category
...
class CategoryTranslate(BaseTranslateMutation):
    ...
    def check_permissions(cls, context, permissions=None):
        ...
        # if is not superuser or staff, raise PermissionDenied
        if not context.user.is_superuser and not context.user.is_staff:
...
class ProductTranslate(BaseTranslateMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        # edit the product if you are the owner of the store or if you are an administrator.
        requestor = info.context.user
        # if user is anon, raise PermissionDenied
        if requestor.is_anonymous:
            raise PermissionDenied()

        if not requestor.is_company_manager(product.company.pk) and not requestor.is_superuser:
            raise PermissionDenied()
...
class ProductVariantTranslate(BaseTranslateMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        # edit the variant if you are the owner of the store or if you are an administrator.
        
        if not requestor.is_company_manager(variant.product.company.pk) and not requestor.is_superuser:
            raise PermissionDenied()
