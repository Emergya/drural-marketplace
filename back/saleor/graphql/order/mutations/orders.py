...
from ....order.actions import (
    ...
    mark_order_as_paid,
    mark_order_as_paid_with_amount,
    order_captured,
    order_confirmed,
    order_refunded,
...
from ....payment import ChargeStatus, PaymentError, TransactionKind, gateway
...
from ..utils import (
    ...
    validate_order_expired_reservation,
)
...
def get_webhook_handler_by_order_status(status, info):
    if status == OrderStatus.DRAFT:
        # FIXME: temporary blocked for draft order status
...
class OrderUpdate(DraftOrderCreate):
    ...
    @classmethod
    def check_ownership_order(cls, info, instance):
        requestor = get_user_or_app_from_context(info.context)
        companies = Company.objects.filter(products__variants__order_lines__order=instance).distinct()
    ...
    def clean_input(cls, info, instance, data):
        cls.check_ownership_order(info, instance)
...
class OrderUpdateShipping(EditableOrderValidationMixin, BaseMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        requestor = get_user_or_app_from_context(info.context)
        companies_ids = set(
            [line.variant.product.company.id for line in order.lines.all()]
        )
...
class OrderCancel(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        if not all(is_your_company) and not requestor.is_staff and not requestor == order.user:
            raise PermissionDenied()
...
class OrderCapture(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, amount, **data):
        ...
        order = cls.get_node_or_error(info, data.get("id"), only_type=Order)
        cls.check_ownership_order(info, order)
        payment = order.get_last_payment()
        cls.clean_billing_address(order)
        if payment is not None and (payment.payment_method_type == "card"):
            ...
            # if this is a payment for transaction, store, or other
            # payment method not supported by gateway
            if payment is not None:
                amount += payment.get_captured_amount().amount
            user = info.context.user
            app = info.context.app
...
class OrderVoid(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        if (
            ...
            or payment.gateway == "mirumee.payments.dummy"
        ):
        ...
        if (
            payment.payment_method_type != "card"
            and payment.gateway != "mirumee.payments.dummy"
        ):
        ...
        if (
            payment.payment_method_type != "card"
            or transaction.kind == TransactionKind.VOID
        ):
            order_voided(
                order,
                info.context.user,
...
class OrderLineDelete(EditableOrderValidationMixin, BaseMutation):
    ...
    def check_ownership_order(cls, info, instance):
        ...
        companies = Company.objects.filter(products__variants__order_lines=instance).distinct()
        is_your_company = map(requestor.is_company_manager, companies.values_list('id', flat=True))
    ...
    def perform_mutation(cls, _root, info, id):
        ...
        cls.check_ownership_order(info, line)
        order = line.order
