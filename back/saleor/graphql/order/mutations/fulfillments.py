...
class OrderFulfill(BaseMutation):
    ...
    def clean_input(cls, data):
        ...
        stock = [line.get("stocks", False) for line in lines]

        for i, line in enumerate(order_lines):
            hasattr(line, "stocks")
            if stock[i] is False and (hasattr(line.variant, "product") and line.variant.product.skip_stock is False):
                raise ValidationError(
                    {
                        "lines": ValidationError(
                            "This product requires stock",
                            code=OrderErrorCode.INVALID
                        )
                    }
                )
        for i, line in enumerate(lines):
            if stock[i] is False and (hasattr(order_lines[i].variant, "product") and order_lines[i].variant.product.skip_stock):
                default_warehouse = models.Warehouse.objects.get(name='Default')
                line["stocks"] = [{"quantity": 1, "warehouse": default_warehouse.pk}]
                stock_default = Stock.objects.get_or_create(
                    product_variant=order_lines[i].variant,
                    warehouse=default_warehouse,
                    defaults={"quantity": 1}
                )
                order_lines[i].variant.stocks.add(stock_default[0])
                order_lines[i].variant.save()
        ...
        for line, order_line in zip(lines, order_lines):
            for stock in line["stocks"]:
                if stock["quantity"] > 0:
                    if (hasattr(order_line.variant, "product") and order_line.variant.product.skip_stock):
                        warehouse_pk = default_warehouse.pk
                    else:
    ...
    @classmethod
    def check_ownership_order(cls, _root, info, order, **data):
        requestor = get_user_or_app_from_context(info.context)

        companies = Company.objects.filter(products__variants__order_lines__order=order).distinct()
        is_your_company = map(requestor.is_company_manager, companies.values_list('id', flat=True))
    ...
    def perform_mutation(cls, _root, info, order, **data):
        ...
        data = data.get("input")
        cls.check_ownership_order(_root, info, order, **data)
        ...
        # Remove stock and relationship to order
        for line in order.lines.all():
            if  hasattr(line.variant, "product") and line.variant.product.skip_stock:
                Stock.objects.filter(product_variant=line.variant).delete()
                line.variant.save()
...
class FulfillmentUpdateTracking(BaseMutation):
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        cls.check_ownership_order(_root, info, fulfillment.order, **data)
...
class FulfillmentCancel(BaseMutation):
    ...
    @classmethod
    def clean_input(cls, _root, info, data):
        fulfillment = cls.get_node_or_error(info, data.get("id"), only_type=Fulfillment)
        line = fulfillment.lines.first()
        warehouse_id = data.get("input", False)
        if warehouse_id is False and hasattr(line.order_line.variant, "product") and line.order_line.variant.product.skip_stock is False:
            raise ValidationError(
                {
                    "lines": ValidationError(
                        "This product requires stock",
        ...
        if warehouse_id is False and hasattr(line.order_line.variant, "product") and line.order_line.variant.product.skip_stock is True:
            warehouse = models.Warehouse.objects.get(name='Default')
            data['input'] = {'warehouse_id': warehouse.id}
            stock_default = Stock.objects.get_or_create(
                product_variant=line.order_line.variant,
                warehouse=warehouse)
            line.order_line.variant.stocks.add(stock_default[0])
            line.order_line.variant.save()
    ...
    @classmethod
    def perform_mutation(cls, _root, info, **data):
        cls.clean_input(_root, info, data)
        fulfillment = cls.get_node_or_error(info, data.get("id"), only_type=Fulfillment)
        warehouse_id = data.get("input").get("warehouse_id")

        if hasattr(fulfillment.lines.first().order_line.variant, "product") and fulfillment.lines.first().order_line.variant.product.skip_stock:
            warehouse = models.Warehouse.objects.get(name='Default')
        else:
        ...
        line = fulfillment.lines.first()
        if hasattr(line.order_line.variant, "product") and line.order_line.variant.product.skip_stock:
            Stock.objects.filter(product_variant=line.order_line.variant).delete()
            line.order_line.variant.save()
...
class FulfillmentRefundProducts(FulfillmentRefundAndReturnProductBase):
    ...
    def check_ownership_order(cls, _root, info, order, **data):
        requestor = get_user_or_app_from_context(info.context)
        this_order = cls.get_node_or_error(info, order, field="order", only_type=Order)

        companies = Company.objects.filter(products__variants__order_lines__order=this_order).distinct()
        is_your_company = map(requestor.is_company_manager, companies.values_list('id', flat=True))
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        cls.check_ownership_order(_root, info, **data)
