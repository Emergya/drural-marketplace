...
from ....order.notifications import convert_to_current_currency, current_currency, get_default_order_payload
from ....payment import ChargeStatus, PaymentError, TransactionKind
from ....payment.models import Payment
from ....plugins.manager import PluginsManager
from ....product.models import Booking, ProductVariant, ProductVariantChannelListing
...
@pytest.fixture
def orders_query_with_filter_channel():
    query = """
    query ($channel: String) {
        orders(first: 5, channel: $channel) {
        totalCount
        edges {
            node {
            id
...
@pytest.fixture
def orders(customer_user, channel_EUR):
...
def test_order_query(
    ...
    # given
    order = fulfilled_order
    net = Money(amount=Decimal("10"), currency="EUR")
    gross = Money(amount=net.amount * Decimal(1.23), currency="EUR").quantize()
...
def test_order_query_payment_list(
    staff_api_client,
    permission_manage_orders,
    fulfilled_order_with_payment,
    shipping_zone
):
    # given
    order = fulfilled_order_with_payment
    net = Money(amount=Decimal("10"), currency="EUR")
    gross = Money(amount=net.amount * Decimal(1.23), currency="EUR").quantize()
    shipping_price = TaxedMoney(net=net, gross=gross)
    order.shipping_price = shipping_price
    ...
    payment_data = order_data["payments"][0]
    assert payment_data["id"] == graphene.Node.to_global_id("Payment", payment.id)
    assert payment_data["capturedAmount"]["amount"] == float(payment.captured_amount)
    assert payment_data["druralFee"]["amount"] == float(payment.drural_fee)
    assert payment_data["totalSeller"]["amount"] == float(payment.captured_amount - payment.drural_fee)
...
def test_order_query_in_eur_channel(
    staff_api_client,
    permission_manage_orders,
    order_with_lines,
    shipping_zone,
...
def test_order_query_without_available_shipping_methods(
    ...
    order.shipping_method = shipping_method_channel_EUR
    order.save()
...
def test_order_confirm_as_user_no_permission(
    user_api_client,
    order_unconfirmed,
    permission_manage_orders,
    payment_txn_preauth,
...
def test_orders_with_channel(
    staff_api_client, permission_manage_orders, orders, channel_EUR
):
    query = """
...
def test_query_order_as_app(app_api_client, permission_manage_orders, order_with_lines):
    query = """
    query OrderQuery($id: ID!) {
        order(id: $id) {
            token
    ...
    assert order_data["token"] == order_with_lines.token
...
def test_draft_order_create_other_company(
    user_api_client,
    permission_manage_orders,
    staff_user,
    customer_user,
...
def test_draft_order_create_other_company_by_staff(
    staff_api_client,
    permission_manage_orders,
    staff_user,
    customer_user,
    product_without_shipping,
    shipping_method,
    variant_other_company,
    voucher,
    channel_EUR,
    graphql_address_data,
):
    variant_0 = variant_other_company
    query = DRAFT_ORDER_CREATE_MUTATION
...
def test_draft_order_create_with_channel(
    ...
    order = Order.objects.first()
    assert order.user == customer_user
    assert order.channel.id == channel_EUR.id
...
def test_draft_order_update_existing_channel_id(
    staff_api_client, permission_manage_orders, order_with_lines, channel_EUR
):
    order = order_with_lines
...
def test_draft_order_update_other_company(
    user_api_client,
    permission_manage_orders,
...
def test_draft_order_update_other_company_by_staff(
    staff_api_client,
    permission_manage_orders,
    order_with_lines_other_company,
    channel_EUR,
...
def test_draft_order_delete_other_company(
    user_api_client, permission_manage_orders, draft_order
):
    order = draft_order
    query = """
        mutation draftDelete($id: ID!) {
...
def test_draft_order_delete_other_company_by_staff(
    staff_api_client, permission_manage_orders, draft_order_other_company
):
    order = draft_order_other_company
    query = """
        mutation draftDelete($id: ID!) {
            draftOrderDelete(id: $id) {
                order {
...
def test_draft_order_complete_other_company(
    user_api_client,
    permission_manage_orders,
    draft_order,
):
...
def test_draft_order_complete_other_company_by_staff(
    staff_api_client,
    permission_manage_orders,
    staff_user,
    draft_order_other_company,
):
    order = draft_order_other_company
...
def test_order_lines_create(
    ...
    #     order, status,  None, order_updated_webhook_mock
    # )
    if status == OrderStatus.DRAFT:
        order_updated_webhook_mock.assert_not_called()
...
def test_order_lines_create_with_existing_variant(
    ...
    # assert_proper_webhook_called_once(
    #     order, status, order_updated_webhook_mock
    # )
    if status == OrderStatus.DRAFT:
        order_updated_webhook_mock.assert_not_called()
...
@patch("saleor.plugins.manager.PluginsManager.order_updated")
def test_order_update_other_company_as_staff(
    order_updated_webhook_mock,
    staff_api_client,
    permission_manage_orders,
    order_with_lines_other_company,
...
def test_order_update_other_company_as_seller(
    seller_api_client,
    permission_manage_orders,
    order_with_lines,
    graphql_address_data,
    ...
    order_id = graphene.Node.to_global_id("Order", order.id)
    variables = {"id": order_id, "email": email, "address": graphql_address_data}
    response = seller_api_client.post_graphql(
        ORDER_UPDATE_MUTATION, variables, permissions=[permission_manage_orders]
    )
    assert_no_permission(response)
...
@patch("saleor.plugins.manager.PluginsManager.order_updated")
def test_order_update_as_seller(
    order_updated_webhook_mock,
    seller_api_client,
    permission_manage_orders,
    order_with_lines_seller_company,
    graphql_address_data,
):
    order = order_with_lines_seller_company
    order.user = None
    order.save()
    email = "not_default@example.com"
    assert not order.user_email == email
...
def test_order_cancel_fulfilled_as_user(
    user_api_client,
    permission_manage_orders,
    order_with_lines,
):
    Fulfillment.objects.create(order=order_with_lines, status=FulfillmentStatus.FULFILLED)
    ...
    content = get_graphql_content_from_response(response)
    data = content["data"]["orderCancel"]
    assert data['errors'][0]['message'] == "This order can't be canceled."


def test_order_cancel_expired_booking_as_user(
    user_api_client,
    permission_manage_orders,
    order_with_lines,
):
    Booking.objects.create(order=order_with_lines, start_date=timezone.now() - timedelta(days=1), end_date=timezone.now() - timedelta(days=1))
    ...
    content = get_graphql_content_from_response(response)
    assert content['data']['orderCancel']['errors'][0]['code'] == 'CANNOT_CANCEL_ORDER'


@patch("saleor.graphql.order.mutations.orders.cancel_order")
@patch("saleor.graphql.order.mutations.orders.clean_order_cancel")
def test_order_cancel_unexpired_booking(
    mock_clean_order_cancel,
    mock_cancel_order,
    staff_api_client,
    permission_manage_orders,
...
def test_order_cancel_other_company(
    user_api_client, permission_manage_orders, order_with_lines_other_company, staff_user
):
    order = order_with_lines_other_company
...
@patch("saleor.graphql.order.mutations.orders.cancel_order")
@patch("saleor.graphql.order.mutations.orders.clean_order_cancel")
def test_order_cancel_other_company_by_staff(
    mock_clean_order_cancel,
    mock_cancel_order,
    staff_api_client,
    permission_manage_orders,
...
@pytest.mark.skip(
    reason="this action cannot be performed, because there\
        is no connection between User and App."
)
@patch("saleor.graphql.order.mutations.orders.cancel_order")
@patch("saleor.graphql.order.mutations.orders.clean_order_cancel")
...
def test_order_capture(
    ...
    payment_txn_preauth.payment_method_type = 'card'
    payment_txn_preauth.save()
...
@mock.patch("saleor.plugins.manager.PluginsManager.notify")
def test_order_capture_as_user_no_permissions(
    mocked_notify,
    channel_EUR,
    user_api_client,
    permission_manage_orders,
...
@mock.patch("saleor.plugins.manager.PluginsManager.notify")
def test_order_capture_as_user(
    mocked_notify,
    channel_EUR,
    seller_api_client,
    permission_manage_orders,
    ...
    payment_dummy.order = order_with_lines_seller_company
    payment_dummy.save()
    ...
    """
    order_id = graphene.Node.to_global_id("Order", order.id)
    amount = float(payment_dummy.total)
    payment_dummy.payment_method_type = 'card'
    payment_dummy.save()
    variables = {"id": order_id, "amount": amount}
    response = seller_api_client.post_graphql(
        query, variables, permissions=[permission_manage_orders]
    ...
    assert event_captured.type == order_events.OrderEvents.PAYMENT_CAPTURED
    assert event_captured.user == seller_user
    assert event_captured.parameters == {
        "amount": str(amount),
        "payment_gateway": "mirumee.payments.dummy",
        "payment_id": "",
    ...
    assert event_order_fully_paid.type == order_events.OrderEvents.ORDER_FULLY_PAID
    assert event_order_fully_paid.user == seller_user
...
def test_order_refund_other_company(
    user_api_client, permission_manage_orders, payment_txn_captured_other_company
):
    order = payment_txn_captured_other_company.order
    ...
    """
    order_id = graphene.Node.to_global_id("Order", order.id)
    amount = float(payment_txn_captured_other_company.total)
...
def test_order_refund_other_company_by_staff(
    staff_api_client, permission_manage_orders, payment_txn_captured_other_company
):
    order = payment_txn_captured_other_company.order
    query = """
        mutation refundOrder($id: ID!, $amount: PositiveDecimal!) {
    ...
    assert refunded_fulfillment
    assert (
        refunded_fulfillment.total_refund_amount
        == payment_txn_captured_other_company.total
    )
    assert refunded_fulfillment.shipping_refund_amount is None
...
@pytest.mark.parametrize(
    ...
    ((True, "orderRefund"), (False, "orderVoid")),
)
...
def test_order_by_token_query_by_app(
    admin_app_api_client, order, customer_user, permission_manage_orders
):
    # given
    query = ORDER_BY_TOKEN_QUERY
    ...
    admin_app_api_client.app.permissions.add(permission_manage_orders)

    order_id = graphene.Node.to_global_id("Order", order.id)

    # when
    response = admin_app_api_client.post_graphql(query, {"token": order.token})
...
def test_query_order_without_addresses(order, user_api_client, channel_EUR):
    # given
    query = ORDER_BY_TOKEN_QUERY
...
def test_order_query_address_without_order_user(
    staff_api_client, permission_manage_orders, channel_EUR, address
):
    query = ORDER_BY_TOKEN_QUERY
...
def test_order_bulk_cancel_fulfilled(
    staff_api_client,
    fulfilled_order_with_all_cancelled_fulfillments,
    permission_manage_orders,
    order_with_lines_list,
    staff_user,
):
    for order in order_with_lines_list:
        Fulfillment.objects.create(order=order, status=FulfillmentStatus.FULFILLED)
        order.user = staff_user
        order.save()
    orders = order_with_lines_list
    orders.append(fulfilled_order_with_all_cancelled_fulfillments)
    variables = {
        "ids": [graphene.Node.to_global_id("Order", order.id) for order in orders],
    }
...
def test_order_bulk_cancel_expired_booking_as_user(
    user_api_client,
    fulfilled_order_with_all_cancelled_fulfillments,
    permission_manage_orders,
    order_with_lines_list,
    ...
    for order in order_with_lines_list:
        Booking.objects.create(order=order, start_date=timezone.now() - timedelta(days=1), end_date=timezone.now() - timedelta(days=1))
        order.user = user_api_client.user
        order.save()
    orders = order_with_lines_list
    orders.append(fulfilled_order_with_all_cancelled_fulfillments)
    ...
    content = get_graphql_content_from_response(response)
    assert content['errors'][0]['message'] == "['You cannot cancel a booking that has already expired.']"


@patch("saleor.graphql.order.bulk_mutations.orders.cancel_order")
def test_order_bulk_cancel_unexpired_booking_as_user(
    mock_cancel_order,
    user_api_client,
    fulfilled_order_with_all_cancelled_fulfillments,
    permission_manage_orders,
    ...
    calls = [
        call(order=order, user=user_api_client.user, app=None, manager=ANY)
        for order in orders
    ]
...
def test_order_bulk_cancel_other_company(
    user_api_client,
    staff_user,
    fulfilled_order_with_all_cancelled_fulfillments,
    permission_manage_orders,
...
@patch("saleor.graphql.order.bulk_mutations.orders.cancel_order")
def test_order_bulk_cancel_one_other_company_by_staff(
    mock_cancel_order,
    staff_api_client,
    permission_manage_orders,
    order_with_lines_other_company,
    ...
    order = order_with_lines_other_company
    variables = {
        "ids": graphene.Node.to_global_id("Order", order.id),
    }
    response = staff_api_client.post_graphql(
        MUTATION_ORDER_BULK_CANCEL, variables, permissions=[permission_manage_orders]
    )
    ...
    mock_cancel_order.call_count == 1


@pytest.mark.skip(
    reason="no way of currently testing this,because today \
        there is no way to connect the user to the APP."
)
@patch("saleor.graphql.order.bulk_mutations.orders.cancel_order")
...
def test_order_query_with_filter_channels_with_one_channel(
    ...
    # given
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.pk)
...
def test_order_query_with_filter_channels_with_many_channel(
    ...
    # given
    Order.objects.create(channel=other_channel_EUR)
    channel_EUR_id = graphene.Node.to_global_id("Channel", channel_EUR.pk)
    channel_EUR_id = graphene.Node.to_global_id("Channel", channel_EUR.pk)
    variables = {"filter": {"channels": [channel_EUR_id, channel_EUR_id]}}
...
def test_order_query_with_filter_channels_with_empty_channel(
    orders_query_with_filter_channel,
    staff_api_client,
    permission_manage_orders,
    orders,
    other_channel_EUR,
    ...
    # given
    variables = {"channel": other_channel_EUR.slug}
...
def test_order_query_with_filter_created(
    ...
    Order.objects.create(channel=channel_EUR)
...
def test_order_query_with_filter_payment_status(
    ...
    payment_dummy.order = Order.objects.create(channel=channel_EUR)
...
def test_order_query_with_filter_customer_fields(
    ...
    order = Order(user=customer_user, token=str(uuid.uuid4()), channel=channel_EUR)
    Order.objects.bulk_create(
        [order, Order(token=str(uuid.uuid4()), channel=channel_EUR)]
    )
...
def test_draft_order_query_with_filter_customer_fields(
    ...
    Order.objects.bulk_create(
        [
            order,
            Order(
                token=str(uuid.uuid4()), status=OrderStatus.DRAFT, channel=channel_EUR
            ),
        ]
    )
...
def test_draft_order_query_with_filter_created_(
    ...
    Order.objects.create(status=OrderStatus.DRAFT, channel=channel_EUR)
    with freeze_time("2012-01-14"):
        Order.objects.create(status=OrderStatus.DRAFT, channel=channel_EUR)
    variables = {"filter": orders_filter}
    staff_api_client.user.user_permissions.add(permission_manage_orders)
...
def test_query_orders_with_sort(
    ...
    created_orders.append(
        Order.objects.create(
            ...
            status=OrderStatus.UNCONFIRMED,
            total=TaxedMoney(net=Money(60, "EUR"), gross=Money(80, "EUR")),
            channel=channel_EUR,
        )
    )
...
def test_orders_query_with_filter_search(
    ...
    net = variant.get_price(
        ...
        channel_EUR,
        channel_listening,
        variant.product.categories.all(),
    )
...
def test_order_query_with_filter_search_by_product_sku_multi_order_lines(
    ...
    net = variants[0].get_price(
        product, [], channel, channel_listening, product.categories.all()
    )
    currency = net.currency
    gross = Money(amount=net.amount * Decimal(1.23), currency=currency)
    quantity = 3
