...
from ...tests.utils import get_graphql_content, assert_no_permission
...
def test_fulfillment_return_products_order_lines_other_company(
    user_api_client,
    permission_manage_orders,
    ...
    payment_dummy.payment_method_type = 'card'
    payment_dummy.total = order_with_lines_other_company.total_gross_amount
    payment_dummy.captured_amount = payment_dummy.total
    payment_dummy.charge_status = ChargeStatus.FULLY_CHARGED
    payment_dummy.save()
    order_with_lines_other_company.payments.add(payment_dummy)
    line_to_return = order_with_lines_other_company.lines.first()
    line_quantity_to_return = 2

    line_to_replace = order_with_lines_other_company.lines.last()
    line_quantity_to_replace = 1

    order_id = graphene.Node.to_global_id("Order", order_with_lines_other_company.pk)
    line_id = graphene.Node.to_global_id("OrderLine", line_to_return.pk)
    replace_line_id = graphene.Node.to_global_id("OrderLine", line_to_replace.pk)
    ...
    response = user_api_client.post_graphql(ORDER_FULFILL_RETURN_MUTATION, variables)

    assert_no_permission(response)


@patch("saleor.order.actions.gateway.refund")
def test_fulfillment_return_products_order_lines_other_company_by_staff(
    mocked_refund,
    staff_api_client,
    permission_manage_orders,
    order_with_lines_other_company,
    ...
    order_with_lines_other_company.refresh_from_db()
    order_return_fulfillment = order_with_lines_other_company.fulfillments.get(
        status=FulfillmentStatus.REFUNDED_AND_RETURNED
    )
    return_fulfillment_line = order_return_fulfillment.lines.filter(
        order_line_id=line_to_return.pk
    ...
    order_replace_fulfillment = order_with_lines_other_company.fulfillments.get(
        status=FulfillmentStatus.REPLACED
    )
    replace_fulfillment_line = order_replace_fulfillment.lines.filter(
        order_line_id=line_to_replace.pk
    ...
    amount = line_to_return.unit_price_gross_amount * line_quantity_to_return
    amount += order_with_lines_other_company.shipping_price_gross_amount
    mocked_refund.assert_called_with(
        payment_dummy, ANY, amount=amount, channel_slug=order_with_lines_other_company.channel.slug
    )
