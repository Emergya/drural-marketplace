...
def test_fulfillment_refund_products_order_lines_other_company(
    user_api_client,
    permission_manage_orders,
    ...
    line_to_refund = order_with_lines_other_company.lines.first()
    ...
    response = user_api_client.post_graphql(ORDER_FULFILL_REFUND_MUTATION, variables)
    
    assert_no_permission(response)


@patch("saleor.order.actions.gateway.refund")
def test_fulfillment_refund_products_order_lines_other_company_by_staff(
    mocked_refund,
    staff_api_client,
    permission_manage_orders,
    order_with_lines_other_company,
    ...
    mocked_refund.assert_called_with(
        ...
        ANY,
        amount=line_to_refund.unit_price_gross_amount * 2,
        channel_slug=order_with_lines_other_company.channel.slug,
    )
