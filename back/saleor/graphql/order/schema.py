...
class OrderQueries(graphene.ObjectType):
    ...
    orders_total_quantity = graphene.Int(
        description="Return the total quantity from a specific period.",
        start_date=graphene.Date(
            description="Start date of the time range", required=True
        ),
        end_date=graphene.Date(description="End date of the time range", required=True),
        channel=graphene.Argument(
            graphene.String,
            description="Slug of a channel for which the data should be returned.",
        ),
    ...
    def resolve_homepage_events(self, info, *_args, **_kwargs):
    ...
    def resolve_order(self, info, **data):
        ...
        if order.status == OrderStatus.DRAFT:
            return order
        company = order.lines.all().first().variant.product.company
        if requestor_is_staff_and_has_permissions(
            requestor, [OrderPermissions.MANAGE_ORDERS]
        ) or requestor.is_company_manager(company.id):
            return order
        else:
            raise PermissionDenied()

    @permission_required(OrderPermissions.MANAGE_ORDERS)
    def resolve_orders(self, info, channel=None, company=None, **_kwargs):
        company_id = None
        requestor = get_user_or_app_from_context(info.context)
        ...
        if company:
            ...
            if not (requestor.is_company_manager(company_id) or requestor.is_staff):
                raise PermissionDenied()
        return resolve_orders(info, requestor, channel, company_id=company_id)

    @permission_required(OrderPermissions.MANAGE_ORDERS)
    def resolve_draft_orders(self, info, company=None, **_kwargs):
        company_id = None
        requestor = get_user_or_app_from_context(info.context)
        if requestor_is_seller(requestor) and company is None:
            raise ValidationError(
        ...
        if company:
            ...
            if not (requestor.is_company_manager(company_id) or requestor.is_staff):
                raise PermissionDenied()
        return resolve_draft_orders(info, company_id=company_id)

    @permission_required(OrderPermissions.MANAGE_ORDERS)
    def resolve_orders_total(self, info, period, channel=None, company=None, **_kwargs):
        company_id = None
        requestor = get_user_or_app_from_context(info.context)
        if requestor_is_seller(requestor) and company is None:
            raise ValidationError(
        ...
        if company:
            ...
            if not (requestor.is_company_manager(company_id) or requestor.is_staff):
                raise PermissionDenied()
        return resolve_orders_total(info, period, channel, company_id=company_id)

    @permission_required(OrderPermissions.MANAGE_ORDERS)
    def resolve_orders_total_quantity(
        self, info, start_date, end_date, channel=None, **_kwargs
    ):
        requestor = get_user_or_app_from_context(info.context)
        if not requestor_is_staff_member_or_app(requestor):
            raise PermissionDenied
        return resolve_orders_total_quantity(info, channel, start_date, end_date)
