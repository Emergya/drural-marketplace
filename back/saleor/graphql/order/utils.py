...
def validate_order_expired_reservation(requestor, order: "Order"):
    if order.booking is not None and (not requestor.is_staff or not requestor.is_seller):
        cancellation_deadline = order.booking.start_date
        if cancellation_deadline < timezone.now():
            raise ValidationError(
                "You cannot cancel a booking that has already expired.",
