...
class OrdersByUserLoader(DataLoader):
    ...
    def batch_load(self, keys):
        ...
        orders = Order.objects.filter(user_id__in=user_pks)
        orders_by_user_map = defaultdict(list)
        for order in orders:
            if (
                requestor.is_staff
                or requestor == order.user
                or order.lines.first().variant.product.company
                in requestor.companies.all()
            ):
                orders_by_user_map[order.user_id].append(order)
        return [orders_by_user_map.get(user_id, []) for user_id in user_pks]
