...
from ...payment import PAYMENTS_OUTSIDE_PLATFORM, PaymentError, gateway
...
class PaymentMethodInput(graphene.InputObjectType):
    ...
    is_active = graphene.Boolean(
        description="Determines if payment method is active",
    )
...
class CheckoutPaymentCreate(BaseMutation, I18nMixin):
    ...
    def perform_mutation(cls, _root, info, checkout_id=None, token=None, **data):
        ...
        if gateway not in PAYMENTS_OUTSIDE_PLATFORM:
...
class PaymentMethodCreate(ModelMutation):
    class Arguments:
        input = paymentMethodCreateInput(
            required=True, description="Fields required to create payment method."
        )

    class Meta:
        description = "Handle Payment Methods."
...
class PaymentMethodUpdate(PaymentMethodCreate):
    class Arguments:
        id = graphene.ID(required=True, description="ID of a payment method to update.")
...
class PaymentMethodDelete(ModelDeleteMutation):
    class Arguments:
