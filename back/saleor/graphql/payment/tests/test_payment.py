...
def test_payments_query(
    ...
    payment_txn_captured.payment_method_type = "card"
    response = staff_api_client.post_graphql(
...
def test_list_payment_sources(
    mocker, dummy_customer_id, set_dummy_customer_id, user_api_client, channel_EUR
):
    gateway = DUMMY_GATEWAY
    query = """
    {
...
def test_payment_initialize(mocked_initialize_payment, api_client, channel_EUR):
...
def test_payment_initialize_gateway_doesnt_exist(api_client, channel_EUR):
    query = PAYMENT_INITIALIZE_MUTATION
    variables = {
...
def test_payment_initialize_plugin_raises_error(
    mocked_initialize_payment, api_client, channel_EUR
):
...
def test_payment_check_balance_validate_channel_inactive(
    ...
    channel_EUR.save(update_fields=["is_active"])
...
def test_payment_total_fee(
    staff_api_client, order_with_lines_list_and_payment, permission_manage_orders
):
    query = QUERY_PAYMENT_TOTAL_FEE
    orders = order_with_lines_list_and_payment

    payment = orders[0].payments.first()
    company_id = graphene.Node.to_global_id(
        "CompanyType", payment.order.lines.first().variant.product.company.id
    )
    variables = {
        "company": company_id,
        "gte": str(payment.created.date()),
        "lte": str(payment.created.date()),
    }
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_orders]
    )
    content = get_graphql_content(response)
    data = content["data"]["paymentTotalFee"]

    assert data["currency"] == payment.currency
    assert data["amount"] == sum(
        [order.payments.first().drural_fee for order in orders]
    )
