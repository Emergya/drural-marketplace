...
def test_sale_query(
    ...
    channel_listing_eur = sale.channel_listings.get(channel=channel_EUR)
    content = get_graphql_content(response)
    ...
    assert (
        ...
        == channel_listing_eur.discount_value
    )
...
def test_sale_query_with_channel_slug(
    staff_api_client, sale, permission_manage_discounts, channel_EUR
):
    query = """
...
def test_query_sales_with_filter_search(
    ...
    SaleChannelListing.objects.bulk_create(
        [
            SaleChannelListing(channel=channel_EUR, discount_value=values[i], sale=sale)
...
def test_query_sales_with_sort(
    sale_sort, result_order, staff_api_client, permission_manage_discounts, channel_EUR
):
    sales = Sale.objects.bulk_create(
        [
            Sale(name="BigSale", type="PERCENTAGE"),
