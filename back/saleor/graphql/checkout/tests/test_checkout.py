...
def test_checkout_create_with_default_channel(
    api_client, stock, graphql_address_data, channel_EUR
):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    ...
    assert new_checkout.channel == channel_EUR
...
def test_checkout_create_with_inactive_and_two_active_default_channel(
    api_client, stock, graphql_address_data, channel_EUR, other_channel_EUR
):
    channel_EUR.is_active = False
    channel_EUR.save()
...
def test_checkout_create_with_many_active_default_channel(
    ...
    assert get_graphql_content(response)["data"]["checkoutCreate"]["errors"] == []
...
def test_checkout_create_with_existing_checkout_in_other_channel(
    user_api_client, stock, graphql_address_data, channel_EUR, user_checkout_EUR
):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    test_email = "test@example.com"
...
def test_checkout_create(api_client, stock, graphql_address_data, channel_EUR):
    """Create checkout object using GraphQL API."""
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    test_email = "test@example.com"
...
def test_checkout_create_multiple_warehouse(
    api_client, variant_with_many_stocks, graphql_address_data, channel_EUR
):
...
def test_checkout_create_with_null_as_addresses(api_client, stock, channel_EUR):
    """Create checkout object using GraphQL API."""
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    test_email = "test@example.com"
...
def test_checkout_create_with_variant_without_inventory_tracking(
    api_client, variant_without_inventory_tracking, channel_EUR
):
    """Create checkout object using GraphQL API."""
...
@pytest.mark.skip(reason="Email will not be required anymore")
def test_checkout_create_required_email(api_client, stock, channel_EUR):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    variables = {
        "checkoutInput": {
...
def test_checkout_create_default_email_for_logged_in_customer(
    user_api_client, stock, channel_EUR
):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    variables = {
...
def test_checkout_create_logged_in_customer(user_api_client, stock, channel_EUR):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    variables = {
        "checkoutInput": {
...
def test_checkout_create_logged_in_customer_custom_addresses(
    user_api_client, stock, graphql_address_data, channel_EUR
):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    shipping_address = graphql_address_data
...
def test_checkout_create_check_lines_quantity_multiple_warehouse(
    user_api_client, variant_with_many_stocks, graphql_address_data, channel_EUR
):
    variant = variant_with_many_stocks
...
def test_checkout_create_sets_country_when_no_shipping_address_is_given(
    api_client, variant_with_many_stocks_different_shipping_zones, channel_EUR
):
    variant = variant_with_many_stocks_different_shipping_zones
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    test_email = "test@example.com"
    ...
    checkout = Checkout.objects.first()
    assert checkout.country == channel_EUR.default_country
...
def test_create_checkout_with_unpublished_product(
    user_api_client, checkout_with_item, stock, channel_EUR
):
    variant = stock.product_variant
    product = variant.product
...
def test_checkout_shipping_address_update_changes_checkout_country(
    ...
    variant = variant_with_many_stocks_different_shipping_zones
    checkout = Checkout.objects.create(channel=channel_EUR, currency="EUR")
...
def test_fetch_checkout_invalid_token(user_api_client, channel_EUR):
...
def test_query_with_channel(
    checkouts_list, staff_api_client, permission_manage_checkouts, channel_EUR
):
    query = """
