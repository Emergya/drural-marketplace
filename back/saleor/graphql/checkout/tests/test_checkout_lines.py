...
def test_checkout_lines_add(
    mocked_update_shipping_method, user_api_client, checkout, stock
):
...
@pytest.mark.skip(reason="Checkout only can have one line")
...
def test_checkout_lines_add_with_unavailable_variant(user_api_client, checkout, stock):
    variant = stock.product_variant
    variant.channel_listings.filter(channel=checkout.channel).update(price_amount=None)
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
...
def test_checkout_lines_add_with_insufficient_stock(user_api_client, checkout, stock):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
...
def test_checkout_lines_add_with_zero_quantity(user_api_client, checkout, stock):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
...
def test_checkout_lines_add_no_channel_shipping_zones(user_api_client, checkout, stock):
    variant = stock.product_variant
...
def test_checkout_lines_add_with_unpublished_product(
    user_api_client, checkout, stock, channel_EUR
):
    variant = stock.product_variant
...
def test_checkout_lines_add_too_many(user_api_client, checkout, stock):
    variant = stock.product_variant
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
...
def test_checkout_lines_update_with_unpublished_product(
    user_api_client, checkout_with_item, channel_EUR
):
    checkout = checkout_with_item
    line = checkout.lines.first()
    variant = line.variant
