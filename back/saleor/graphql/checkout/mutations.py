...
from ...checkout.complete_checkout import (
    complete_checkout,
    complete_checkout_paying_outside_platform,
)
...
from ...payment import PAYMENTS_OUTSIDE_PLATFORM
...
def check_lines_number(existing_lines):
    if len(existing_lines) > 0:
        raise ValidationError(
            {
                "quantity": ValidationError(
                    "It is only permitted one line per checkout.",
                    code=CheckoutErrorCode.LINES_QUANTITY,
                )
            }
        )
...
def validate_booking_grace_time(checkout):
    line = checkout.lines.select_related("booking").first()
    booking = line.booking
    if booking and not booking.can_confirm():
        checkout.delete()
        raise ValidationError(
            {
                "booking": ValidationError(
                    "This checkout can't be completed, because "
                    "the grace time of related booking is finished.",
                    code=CheckoutErrorCode.CHECKOUT_WITH_INVALID_BOOKING.value,
                )
            }
        )
...
class CheckoutLineInput(graphene.InputObjectType):
    ...
    booking = graphene.ID(description="ID of booking.")
...
class CheckoutCreate(ModelMutation, I18nMixin):
    ...
    @classmethod
    def validate_variants_billable(cls, variants: Iterable["ProductVariant"]):
        """Check if variant's product is billable."""
        for variant in variants:
            if not variant.product.is_billable:
                raise ValidationError(
                    {
                        "variant": ValidationError(
                            f"This product ({variant.name}) is not billable",
    ...
    def clean_checkout_lines(
        ...
        List[product_models.ProductVariant], List[int], List[product_models.Booking]
    ]:
        ...
        cls.validate_variants_billable(variants)
        quantities = [line["quantity"] for line in lines]
        booking_ids = [line.get("booking", False) for line in lines]
        bookings = []
        if all(booking_ids):
            bookings = cls.get_nodes_or_error(
                booking_ids, "id", "Booking", qs=product_models.Booking.objects.all()
            )
        ...
        return variants, quantities, bookings
    ...
    def save(cls, info, instance: models.Checkout, cleaned_input):
        ...
        bookings = cleaned_input.get("bookings")
...
class CheckoutDelete(ModelDeleteMutation):
    class Arguments:
    ...
    class Meta:
        description = "Delete an existing checkout."
        model = models.Checkout
        return_field_name = "checkout"
        error_type_class = CheckoutError
        error_type_field = "checkout_errors"
    ...
    def perform_mutation(cls, _root, info, **data):
        ...
        if token := data.get("token"):
            instance = get_checkout_by_token(token)
        else:
...
class CheckoutLinesAdd(BaseMutation):
    ...
    def validate_checkout_lines(
        ...
        check_lines_number(existing_lines)
        check_lines_quantity(
            variants, quantities, country, channel_slug, existing_lines=existing_lines
        )
    ...
    def perform_mutation(
        ...
        checkout_lines_info = fetch_checkout_lines(checkout)
        ...
        update_checkout_shipping_method_if_invalid(checkout_info, checkout_lines_info)
        recalculate_checkout_discount(
            manager, checkout_info, checkout_lines_info, info.context.discounts
        )
...
class CheckoutComplete(BaseMutation):
    ...
    def perform_mutation(
        ...
        with transaction_with_commit_on_errors():
            ...
            # Check grace time of booking if exists.
            validate_booking_grace_time(checkout)
            ...
            order, action_required, action_data = (
                complete_checkout_paying_outside_platform(
                    manager=manager,
                    checkout_info=checkout_info,
                    lines=lines,
                ...
                if payment and payment.gateway in PAYMENTS_OUTSIDE_PLATFORM
                else complete_checkout(
                    manager=manager,
                    checkout_info=checkout_info,
                    lines=lines,
