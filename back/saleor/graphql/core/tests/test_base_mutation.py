...
def test_resolve_id(product, schema_context, channel_EUR):
    product_id = graphene.Node.to_global_id("Product", product.pk)
    variables = {"productId": product_id, "channel": channel_EUR.slug}
...
def test_user_error_nonexistent_id(schema_context, channel_EUR):
    variables = {"productId": "not-really", "channel": channel_EUR.slug}
    result = schema.execute(
        TEST_MUTATION, variables=variables, context_value=schema_context
    )
    assert not result.errors
...
def test_mutation_custom_errors_default_value(product, schema_context, channel_EUR):
    product_id = graphene.Node.to_global_id("Product", product.pk)
    query = """
        mutation testMutation($productId: ID!, $channel: String) {
...
def test_user_error_id_of_different_type(product, schema_context, channel_EUR):
    ...
    variables = {"productId": variant_id, "channel": channel_EUR.slug}
    result = schema.execute(
        TEST_MUTATION, variables=variables, context_value=schema_context
    )
    assert not result.errors
