...
def test_user_error_field_name_for_related_object(
    ...
    response = staff_api_client.post_graphql(
        query, permissions=[permission_manage_category]
    )
...
def test_total_count_query(api_client, product, channel_EUR):
    query = """
    ...
    """
    response = api_client.post_graphql(query, {"channel": channel_EUR.slug})
    content = get_graphql_content(response)
