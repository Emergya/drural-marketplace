...
def clean_seo_fields(data):
    ...
    if seo_fields:
        ...
        raw_keywords = seo_fields.get("keywords")
        if raw_keywords is not None:
            # remove whitespace from each string and convert to lowercase
            keywords = [
                keyword.replace(" ", "").lower() for keyword in raw_keywords if keyword
            ]
...
def validate_image_file_banner(file, field_name, error_class):
    width, height = get_image_dimensions(file)
    if width < 1440 or height < 480:
        raise ValidationError(
            {
                field_name: ValidationError(
                    "Image dimensions is too small, minimum is 1440x480",
                    code=error_class.DIMENSION_ERROR,
                )
            }
        )
...
def validate_logo_file(file, field_name, error_class):
    """Validate if the file is an image."""
    if not file:
        raise ValidationError(
            {
    ...
    _validate_logo_format(file, field_name, error_class)


def _validate_logo_format(file, field_name, error_class):
    """Validate image file format."""
    _file_name, format = os.path.splitext(file._name)
    if not format:
        raise ValidationError(
...
def validate_mesurement(ga_tag, error_class):
    if ga_tag.get("measurement_id") == "":
        raise ValidationError(
            {"measurement_id": "Google Analytics ID cannot be empty."},
            code=error_class.INVALID,
        )


def validate_commission(value_commission, error_class):

    if value_commission < 0 or value_commission > 1:
        raise ValidationError(
            {"commission": "Commission must be between 0 and 1."},
            code=error_class.INVALID,
        )
