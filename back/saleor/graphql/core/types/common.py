...
# from ....graphql.app.enums import description
...
class CompanyError(Error):
    code = CompanyErrorCode(description="The error code.", required=True)
...
class BookableResourceError(Error):
    code = BookableResourceErrorCode(description="The error code.", required=True)
...
class RatingError(Error):
    code = ReportErrorCode(description="The error code.", required=True)
...
class ConsumptionRangeInput(graphene.InputObjectType):
    start_date = graphene.Date(description="Start date.", required=True)
    end_date = graphene.Date(description="End date.", required=True)


class ClosenessRangeInput(graphene.InputObjectType):
    radius = graphene.Int(description="Radius kms of distance", required=True)
    latitude = graphene.Float(description="Latitude of the user", required=True)
    longitude = graphene.Float(description="Longitude of the user", required=True)
...
class ReviewPercentages(graphene.ObjectType):
    stars = graphene.Int(description="Rating")
    total = graphene.Int(description="Percentage of reviews with that number of stars")
