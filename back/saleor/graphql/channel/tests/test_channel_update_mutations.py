...
def test_channel_update_mutation_as_staff_user(
    ...
    channel_EUR.refresh_from_db()
    assert channel_data["name"] == channel_EUR.name == name
    assert channel_data["slug"] == channel_EUR.slug == slug
    assert channel_data["currencyCode"] == channel_EUR.currency_code == "EUR"
    assert (
        channel_data["defaultCountry"]["code"]
        == channel_EUR.default_country.code
        == default_country
    )


def test_channel_update_mutation_as_app(
    permission_manage_channels, app_api_client, channel_EUR
):
    # given
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
    name = "newName"
...
def test_channel_update_mutation_as_customer(user_api_client, channel_EUR):
    # given
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
    name = "newName"
    slug = "new_slug"
...
def test_channel_update_mutation_as_anonymous(api_client, channel_EUR):
    # given
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
    name = "newName"
    slug = "new_slug"
...
def test_channel_update_mutation_add_shipping_zone(
    permission_manage_channels, staff_api_client, channel_EUR, shipping_zone
):
    # given
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
    name = "newName"
    ...
    assert channel_data["slug"] == channel_EUR.slug == slug
    assert channel_data["currencyCode"] == channel_EUR.currency_code == "EUR"
    actual_shipping_zone = channel_EUR.shipping_zones.first()
...
def test_channel_update_mutation_remove_shipping_zone(
    ...
    # given
    channel_EUR.shipping_zones.add(*shipping_zones)
    ...
    assert channel_EUR.shipping_method_listings.filter(
    ...
    assert channel_data["slug"] == channel_EUR.slug == slug
    assert channel_data["currencyCode"] == channel_EUR.currency_code == "EUR"
    assert not channel_EUR.shipping_method_listings.filter(
        shipping_method__shipping_zone=shipping_zone
    )
    mocked_drop_invalid_shipping_methods_relations.assert_called_once_with(
        list(shipping_method_ids), [channel_EUR.id]
    )
...
def test_channel_update_mutation_add_and_remove_shipping_zone(
    ...
    assert channel_data["slug"] == channel_EUR.slug == slug
    assert channel_data["currencyCode"] == channel_EUR.currency_code == "EUR"
    zones = channel_EUR.shipping_zones.all()
