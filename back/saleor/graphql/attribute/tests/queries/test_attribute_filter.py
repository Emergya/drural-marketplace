...
def test_filter_attributes_in_category_invalid_category_id(
    user_api_client, product_list, weight_attribute, channel_EUR
):
    # given
...
def test_filter_attributes_in_collection_invalid_category_id(
    user_api_client, product_list, weight_attribute, collection, channel_EUR
):
    # given
    product_type = ProductType.objects.create(
        name="Default Type 2",
...
def test_attributes_filter_by_product_type_with_unsupported_field(
    ...
    with pytest.raises(NotImplementedError) as exc:
        filter_attributes_by_product_types(
            qs, "in_space", "a-value", customer_user, channel_EUR.slug
        )

    assert exc.value.args == ("Filtering by in_space is unsupported",)


def test_attributes_filter_by_non_existing_category_id(customer_user, channel_EUR):
    ...
    qs = filter_attributes_by_product_types(
        mocked_qs, "in_category", category_id, customer_user, channel_EUR.slug
    )
