...
def test_get_variant_pricing_on_sale(api_client, sale, product, channel_EUR):
    ...
    variables = {"channel": channel_EUR.slug, "address": {"country": "ES"}}
...
def test_get_variant_pricing_not_on_sale(api_client, product, channel_EUR):
    price = product.variants.first().channel_listings.get().price
...
def test_variant_pricing(
    variant: ProductVariant, monkeypatch, settings, stock, channel_EUR
):
    ...
    assert pricing.price.currency == "EUR"  # type: ignore
