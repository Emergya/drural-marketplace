...
def test_sort_products_within_collection_invalid_collection_id(
    staff_api_client, collection, product, permission_manage_collection
):
    ...
    content = get_graphql_content(
        staff_api_client.post_graphql(
            ...
            permissions=[permission_manage_collection],
        )
...
def test_sort_products_within_collection(
    ...
    staff_api_client.user.user_permissions.add(permission_manage_collection)
...
def test_sort_products_by_publication_date(
    direction, order_direction, api_client, product_list, channel_EUR
):
    product_channel_listings = []
    for iter_value, product in enumerate(product_list):
        product_channel_listing = product.channel_listings.get(channel=channel_EUR)
