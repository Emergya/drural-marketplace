...
def test_category_query_by_id(user_api_client, product, channel_EUR):
...
def test_category_query_invalid_id(user_api_client, product, channel_EUR):
    category_id = "'"
    variables = {
        "id": category_id,
        "channel": channel_EUR.slug,
...
def test_category_query_description(user_api_client, product, channel_EUR):
    category = Category.objects.first()
...
def test_category_query_without_description(user_api_client, product, channel_EUR):
    category = Category.objects.first()
    category.save()
    variables = {
        "id": graphene.Node.to_global_id("Category", category.pk),
...
def test_category_query_by_slug(user_api_client, product, channel_EUR):
    category = Category.objects.first()
    variables = {"slug": category.slug, "channel": channel_EUR.slug}
    response = user_api_client.post_graphql(QUERY_CATEGORY, variables=variables)
    content = get_graphql_content(response)
    category_data = content["data"]["category"]
    assert category_data is not None
...
def test_category_query_error_when_id_and_slug_provided(
    user_api_client, product, graphql_log_handler, channel_EUR
):
    category = Category.objects.first()
    variables = {
        "id": graphene.Node.to_global_id("Category", category.pk),
...
def test_query_category_product_visible_in_listings_as_staff_without_manage_products(
    ...
    content = get_graphql_content(response, ignore_errors=True)
    assert (
        len(content["data"]["category"]["products"]["edges"]) == product_count
    )  # invisible but staff
...
def test_query_category_product_not_visible_in_listings_as_app_with_perm(
    ...
    """
    dRural update:
    If a product is not visible it will not enough having
    the "permission_manage_products".
    User must be a member of the company to get its information.
    """
    assert len(content["data"]["category"]["products"]["edges"]) == product_count - 1
...
CATEGORY_CREATE_MUTATION = """
        mutation(
                ...
                $description: JSONString, $parentId: ID) {
            categoryCreate(
                input: {
                    name: $name
                    slug: $slug
...
def test_category_create_mutation(
    monkeypatch, staff_api_client, permission_manage_category, media_root
):
    ...
    response = staff_api_client.post_multipart(
        body, permissions=[permission_manage_category]
    )
    content = get_graphql_content(response)
    ...
    assert category.description_plaintext == ""
...
def test_create_category_with_given_slug(
    staff_api_client, permission_manage_category, input_slug, expected_slug
):
    query = CATEGORY_CREATE_MUTATION
    name = "Test category"
    variables = {"name": name, "slug": input_slug}
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_category]
    )
    content = get_graphql_content(response)
    data = content["data"]["categoryCreate"]
...
def test_create_category_name_with_unicode(
    staff_api_client, permission_manage_category
):
    query = CATEGORY_CREATE_MUTATION
    name = "わたし-わ にっぽん です"
    variables = {"name": name}
...
def test_category_create_mutation_without_background_image(
    monkeypatch, staff_api_client, permission_manage_category
):
    query = CATEGORY_CREATE_MUTATION
...
def test_category_update_mutation(
    monkeypatch, staff_api_client, category, permission_manage_category, media_root
):
    ...
    response = staff_api_client.post_graphql(
        MUTATION_CATEGORY_UPDATE_MUTATION,
        variables,
        permissions=[permission_manage_category],
    )
    content = get_graphql_content(response)
...
@pytest.mark.skip(reason="Background images have been removed from the category model")
def test_category_update_mutation_invalid_background_image(
    staff_api_client, category, permission_manage_category
):
...
def test_category_update_mutation_without_background_image(
    monkeypatch, staff_api_client, category, permission_manage_category
):
    query = """
...
def test_category_delete_mutation(
    ...
    response = staff_api_client.post_graphql(
        MUTATION_CATEGORY_DELETE, variables, permissions=[permission_manage_category]
    )
    content = get_graphql_content(response)
...
def test_category_delete_mutation_for_categories_tree(
    ...
    assert product_channel_listings.count() == 2
...
def test_category_delete_mutation_for_children_from_categories_tree(
    ...
    assert parent_product.categories
    product_channel_listings = ProductChannelListing.objects.filter(
    ...
    assert len(child_product.categories.all()) == 0
    product_channel_listings = ProductChannelListing.objects.filter(
...
@pytest.mark.skip(reason="Background images were removed from categories")
def test_update_category_mutation_remove_background_image(
    staff_api_client, category_with_image, permission_manage_category
):
    query = """
