...
@pytest.fixture
def categories_for_pagination(product_type, company):
    categories = Category.tree.build_tree_nodes(
        {
            ...
            "slug": "cat1",
            "children": [
                {"parent_id": 10, "name": "CatCat1", "slug": "cat_cat1"},
                {"parent_id": 10, "name": "CatCat2", "slug": "cat_cat2"},
                {"parent_id": 10, "name": "Cat1", "slug": "cat2"},
                {"parent_id": 10, "name": "Cat3", "slug": "cat3"},
            ],
        }
    )
...
@pytest.mark.parametrize(
    ...
    [
        ({"search": "CatCat"}, ["CatCat1", "CatCat2"]),
        ({"search": "cat_cat"}, ["CatCat1", "CatCat2"]),
    ],
)
...
@pytest.fixture
def collections_for_pagination(product, product_with_single_variant, channel_EUR):
    ...
    CollectionChannelListing.objects.bulk_create(
        [
            CollectionChannelListing(
                channel=channel_EUR, is_published=published[num], collection=collection
            )
...
def products_for_pagination(
    product_type, color_attribute, categories, warehouse, channel_EUR, company
):
...
def test_products_pagination_with_filtering(
    ...
    assert len(products_nodes) == page_size
    assert products_order[0] == products_nodes[0]["node"]["name"]
    assert products_order[1] == products_nodes[2]["node"]["name"]
...
def test_products_pagination_with_filtering_by_attribute(
    staff_api_client, permission_manage_products, products_for_pagination, channel_EUR
):
    page_size = 2
