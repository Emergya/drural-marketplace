...
def test_delete_categories(staff_api_client, category_list, permission_manage_category):
    variables = {
        "ids": [
...
def test_delete_categories_trigger_product_updated_webhook(
    ...
    first_product.categories.set([category_list[0]])
    first_product.save()

    second_product = product_list[1]
    second_product.categories.set([category_list[1]])
    second_product.save()
    ...
    assert product_updated_mock.call_count == 0
...
def test_delete_categories_with_subcategories_and_products(
    ...
    parent_product.id = 2
    parent_product.categories.set([category_list[0]])
    ...
    for product in product_list:
        ...
        assert len(product.categories.all()) == 0
...
def test_delete_collections(
    staff_api_client, collection_list, permission_manage_collection
):
...
def test_delete_product_variants_in_draft_orders(
    ...
    net = second_variant_in_draft.get_price(
        ...
        [],
        channel_EUR,
        second_variant_channel_listing,
        second_product.categories.all(),
        None,
    )
    gross = Money(amount=net.amount, currency=net.currency)
    unit_price = TaxedMoney(net=net, gross=gross)
