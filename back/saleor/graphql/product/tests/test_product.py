...
def test_product_query_by_id_available_as_staff_user(
    staff_api_client, permission_manage_products, product, channel_EUR
):
    variables = {
...
def test_product_query_by_id_not_available_as_staff_user(
    ...
    ProductChannelListing.objects.filter(product=product, channel=channel_EUR).update(
        is_published=False
    )
...
def test_product_query_by_id_not_existing_in_channel_as_staff_user(
    ...
    ProductChannelListing.objects.filter(product=product, channel=channel_EUR).delete()
...
def test_product_query_by_id_available_as_app(
    app_api_client, permission_manage_products, product, channel_EUR
):
    variables = {
        "id": graphene.Node.to_global_id("Product", product.pk),
        "channel": channel_EUR.slug,
...
def test_product_query_by_invalid_id(
    id, staff_api_client, permission_manage_products, product, channel_EUR
):
    variables = {
        "id": id,
        "channel": channel_EUR.slug,
...
def test_product_query_by_id_as_user(
    user_api_client, permission_manage_products, product, channel_EUR
):
...
def test_product_query_invalid_id(user_api_client, product, channel_EUR):
    product_id = "'"
    variables = {
        "id": product_id,
        "channel": channel_EUR.slug,
...
def test_product_query_with_invalid_object_type(user_api_client, product, channel_EUR):
...
def test_get_collections_from_product_as_customer(
    user_api_client, product_with_collections, channel_EUR, published_collection
):
    # given
    product = product_with_collections
    variables = {
...
def test_get_collections_from_product_as_anonymous(
    api_client, product_with_collections, channel_EUR, published_collection
):
    # given
    product = product_with_collections
    variables = {
...
def test_product_unpublished_query_by_id_as_app(
    app_api_client, unavailable_product, permission_manage_products, channel_EUR
):
    # given
    variables = {
...
def test_product_query_by_id_weight_returned_in_default_unit(
    user_api_client, product, site_settings, channel_EUR
):
    # given
...
def test_product_query_by_slug(user_api_client, product, channel_EUR):
    variables = {
        "id": graphene.Node.to_global_id("Product", product.pk),
        "channel": channel_EUR.slug,
    }
...
def test_product_query_by_slug_as_app_without_channel(
    ...
    assert product_data["isAvailableForPurchase"] is None
    assert product_data["isAvailable"] is None
...
def test_product_query_unpublished_products_by_slug_and_anonymous_user(
    api_client, product, channel_EUR
):
    # given
    ProductChannelListing.objects.filter(product=product, channel=channel_EUR).update(
        is_published=False
...
def test_fetch_all_product_variants_available_as_staff_user_with_channel(
    staff_api_client, permission_manage_products, product_variant_list, channel_EUR
):
    variables = {"channel": channel_EUR.slug}
    response = staff_api_client.post_graphql(
        QUERY_FETCH_ALL_PRODUCTS,
...
def test_fetch_all_products_not_existing_in_channel_as_staff_user(
    staff_api_client, permission_manage_products, channel_EUR, product_list
):
    variables = {"channel": channel_EUR.slug}
    ProductChannelListing.objects.filter(
        product=product_list[0], channel=channel_EUR
    ).delete()
...
def test_fetch_all_products_as_staff_user_without_channel_slug(
    staff_api_client, permission_manage_products, product_list, channel_EUR
):
    ProductChannelListing.objects.filter(
        product=product_list[0], channel=channel_EUR
    ).delete()
...
def test_fetch_all_products_not_existing_in_channel_as_app(
    app_api_client, permission_manage_products, product_list, channel_EUR
):
    variables = {"channel": channel_EUR.slug}
    ProductChannelListing.objects.filter(
        product=product_list[0], channel=channel_EUR
...
def test_fetch_all_products_available_as_anonymous(api_client, product, channel_EUR):
    variables = {"channel": channel_EUR.slug}
...
def test_fetch_all_products_visible_in_listings(
    user_api_client, product_list, permission_manage_products, channel_EUR
):
    # given
...
def test_fetch_all_products_visible_in_listings_by_staff_with_perm(
    staff_api_client, product_list, permission_manage_products, channel_EUR
):
    # given
    product_list[0].channel_listings.update(visible_in_listings=False)
...
def test_fetch_all_products_visible_in_listings_by_staff_without_manage_products(
    staff_api_client, product_list, channel_EUR
):
    # given
    product_list[0].channel_listings.update(visible_in_listings=False)
...
def test_fetch_all_products_visible_in_listings_by_app_without_manage_products(
    app_api_client, product_list, channel_EUR
):
    # given
    product_list[0].channel_listings.update(visible_in_listings=False)
...
def test_fetch_product_from_category_query(
    staff_api_client, product, permission_manage_products, stock, channel_EUR
):
...
def test_query_product_is_available_with_one_variant(
    api_client, channel_EUR, product_with_two_variants
):
    # given
...
def test_products_query_with_filter_numeric_attributes(
    ...
    ProductChannelListing.objects.create(
        ...
        available_for_purchase=date(1999, 1, 1),
    )
    attr_value = AttributeValue.objects.create(
...
def test_products_query_filter_by_non_existing_attribute(
    query_products_with_filter, api_client, product_list, channel_EUR
):
    variables = {
        "channel": channel_EUR.slug,
...
def test_products_query_with_filter_category_and_search(
    ...
    category_id = graphene.Node.to_global_id("Category", category.id)
    variables = {"filter": {"categories": [category_id], "search": second_product.name}}
    staff_api_client.user.user_permissions.add(permission_manage_products)
    response = staff_api_client.post_graphql(query_products_with_filter, variables)
    content = get_graphql_content(response)
    product_id = graphene.Node.to_global_id("Product", second_product.id)
    products = content["data"]["products"]["edges"]
...
def test_products_with_variants_query_as_app(
    ...
    response = app_api_client.post_graphql(query_products_with_attributes, variables)
    content = get_graphql_content(response)
    products = content["data"]["products"]["edges"]
...
def test_products_query_with_filter(
    ...
    variables = {"filter": products_filter, "channel": channel_EUR.slug}
    staff_api_client.user.user_permissions.add(permission_manage_products)
    response = staff_api_client.post_graphql(query_products_with_filter, variables)
    content = get_graphql_content(response)
    second_product_id = graphene.Node.to_global_id("Product", second_product.id)
...
def test_products_query_with_filter_search_by_sku(
    ...
    ProductChannelListing.objects.filter(
        product=product_with_default_variant, channel=channel_EUR
...
def test_products_query_with_is_published_filter_variants_without_prices(
    ...
    variables = {"channel": channel_EUR.slug, "filter": {"isPublished": True}}
    response = staff_api_client.post_graphql(
        query_products_with_filter,
        variables,
        permissions=[permission_manage_products],
...
def test_products_query_with_filter_stock_availability_as_staff(
    ...
    product = product_list[0]
    product.variants.first().channel_listings.filter(channel=channel_EUR).update(
        price_amount=None
    )
    variables = {
...
def test_query_products_with_filter_ids(
    api_client, product_list, query_products_with_filter, channel_EUR
):
    # given
    product_ids = [
...
def test_query_product_media_by_id(user_api_client, product_with_image, channel_EUR):
...
def test_query_product_media_by_id_missing_id(
    user_api_client, product_with_image, channel_EUR
):
    query = QUERY_PRODUCT_MEDIA_BY_ID
    variables = {
        "productId": graphene.Node.to_global_id("Product", product_with_image.pk),
...
def test_query_product_image_by_id(user_api_client, product_with_image, channel_EUR):
...
def test_filter_products_by_wrong_attributes(user_api_client, product, channel_EUR):
...
def test_filter_products_with_unavailable_variants_attributes_as_user(
    user_api_client, product_list, channel_EUR
):
...
def test_filter_products_with_unavailable_variants_attributes_as_staff(
    staff_api_client, product_list, channel_EUR, permission_manage_products
):
    product_attr = product_list[0].product_type.product_attributes.first()
    attr_value = product_attr.values.first()
    staff_api_client.user.user_permissions.add(permission_manage_products)
...
def test_sort_products(user_api_client, product, channel_EUR):
...
def test_sort_products_by_price_as_staff(
    staff_api_client, product, channel_EUR, permission_manage_products
):
    product.updated_at = datetime.utcnow()
    product.save()
    staff_api_client.user.user_permissions.add(permission_manage_products)
...
def test_sort_products_product_type_name(
    user_api_client, product, product_with_default_variant, channel_EUR
):
    variables = {"channel": channel_EUR.slug}
...
def test_create_product(
    ...
    company_id = graphene.Node.to_global_id("CompanyType", company.pk)
...
@patch("saleor.plugins.manager.PluginsManager.product_updated")
@patch("saleor.plugins.manager.PluginsManager.product_created")
def test_create_product_with_keywords(
    created_webhook_mock,
    updated_webhook_mock,
    staff_api_client,
    product_type,
    ...
    assert keywords["edges"][0]["node"]["nameKeyword"] == "keyword_1"
    assert keywords["edges"][1]["node"]["nameKeyword"] == "keyword_2"
    assert keywords["edges"][2]["node"]["nameKeyword"] == "keyword_3"
    ...
    assert product.keywords.all()[0].name_keyword == "keyword_1"
    assert product.keywords.all()[1].name_keyword == "keyword_2"
    assert product.keywords.all()[2].name_keyword == "keyword_3"
    assert product.keywords.count() == 3
...
@patch("saleor.plugins.manager.PluginsManager.product_updated")
@patch("saleor.plugins.manager.PluginsManager.product_created")
def test_create_product_with_custom_email(
    created_webhook_mock,
    updated_webhook_mock,
    staff_api_client,
    product_type,
    ...
    assert data["product"]["productType"]["name"] == product_type.name
    assert data["product"]["category"]["name"] == category.name
    assert data["product"]["purchaseEmail"]["subject"] == subject
    assert data["product"]["purchaseEmail"]["title"] == title
    assert data["product"]["purchaseEmail"]["content"] == body
    values = (
        data["product"]["attributes"][0]["values"][0]["slug"],
        data["product"]["attributes"][1]["values"][0]["slug"],
    )
...
def test_create_product_with_date_time_attribute(
    ...
    product_type_id = graphene.Node.to_global_id("ProductType", product_type.pk)
    company_id = graphene.Node.to_global_id("CompanyType", company.pk)
    category_id = graphene.Node.to_global_id("Category", product.category.id)
    product_name = "test name"
...
def test_search_product_by_description(user_api_client, product_list, channel_EUR):

    variables = {"filters": {"search": "big"}, "channel": channel_EUR.slug}
    response = user_api_client.post_graphql(SEARCH_PRODUCTS_QUERY, variables)
    content = get_graphql_content(response)
    assert len(content["data"]["products"]["edges"]) == 2

    variables = {"filters": {"search": "small"}, "channel": channel_EUR.slug}
    response = user_api_client.post_graphql(SEARCH_PRODUCTS_QUERY, variables)
    content = get_graphql_content(response)
...
def test_search_product_by_description_and_name(
    user_api_client, product_list, product, channel_EUR, category, product_type
):
    ...
    assert len(data) == 3
    assert data[2]["node"]["name"] == product_2.name
...
def test_search_product_by_description_and_name_and_use_cursor(
    ...
    product_2.save()
    product_1 = product_list[0]
    product_1.name = "some red product"
    product_1.save()
    ...
    assert data[1]["node"]["name"] == product_2.name


def test_search_product_by_keywords(
    user_api_client, product_list, product, channel_EUR, category, product_type
):
    # create keywords
    Keyword.objects.create(name_keyword="red")
    keyword_forest = Keyword.objects.create(name_keyword="forest")

    product.keywords.add(keyword_forest)
    product.save()
...
def test_create_product_no_category_id(
    ...
    assert data["product"]["taxType"]["taxCode"] == product_tax_rate
    assert data["product"]["productType"]["name"] == product_type.name
    assert data["product"]["category"]["name"] == "Default"
...
def test_product_create_without_product_type(
    staff_api_client, category, company, permission_manage_products
):
    query = """
    mutation createProduct($category: ID!, $company: ID!) {
        productCreate(input: {
                name: "Product",
                company: $company,
                category: $category}) {
    ...
    response = staff_api_client.post_graphql(
        query,
        {"category": category_id, "company": company_id},
        permissions=[permission_manage_products],
    )
    content = get_graphql_content(response)
    assert content["data"]["productCreate"]["errors"] == []
    assert (
        content["data"]["productCreate"]["product"]["productType"]["name"] == "Simple"
    )
...
def test_update_product(
    ...
    payment_method_id = graphene.Node.to_global_id(
        "PaymentMethod", card_payment_method.pk
    )
...
@patch("saleor.plugins.manager.PluginsManager.product_updated")
@patch("saleor.plugins.manager.PluginsManager.product_created")
def test_update_product_add_custom_email(
    created_webhook_mock,
    updated_webhook_mock,
    staff_api_client,
    category,
...
@patch("saleor.plugins.manager.PluginsManager.product_updated")
@patch("saleor.plugins.manager.PluginsManager.product_created")
def test_update_product_delete_custom_email(
    created_webhook_mock,
    updated_webhook_mock,
    staff_api_client,
    category,
    ...
    assert data["product"]["taxType"]["taxCode"] == product_tax_rate
    assert not data["product"]["category"]["name"] == category.name
    assert data["product"]["purchaseEmail"] is None
    attributes = data["product"]["attributes"]
...
def test_update_product_with_boolean_attribute_value(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "boolean": new_value}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_with_file_attribute_value(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "file": new_value}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_with_numeric_attribute_value(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "values": [new_value]}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_clear_attribute_values(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "values": []}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_with_page_reference_attribute_value(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "references": [reference]}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_with_empty_input_collections(
    ...
    variables = {
        "productId": product_id,
        "input": {"collections": [""], "category": category_id},
    }
    # when
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_products]
...
def test_update_product_with_page_reference_attribute_value_not_given(
    ...
    variables = {
        "productId": product_id,
        "input": {
            "attributes": [{"id": attribute_id, "values": ["test"]}],
            "category": category_id,
            "paymentMethods": [payment_method_id],
        },
    }
...
def test_update_product_with_keywords(
    user_api_client,
    product,
    permission_manage_products,
    product_type,
    ...
    # create keywords
    keyword_red = Keyword.objects.create(name_keyword="red")
    keyword_forest = Keyword.objects.create(name_keyword="forest")
    ...
    variables = {
        ...
        "input": {
            ...
            "seo": {
                ...
                "keywords": [keyword_red.name_keyword],
            },
        },
    }
    ...
    keywords = content["data"]["productUpdate"]["product"]["keywords"]

    assert keywords["edges"][0]["node"]["nameKeyword"] == keyword_red.name_keyword
    assert len(keywords["edges"]) == 1


def test_update_product_with_keywords_not_created(
    user_api_client,
    product,
    permission_manage_products,
    product_type,
    ...
    assert keywords["edges"][0]["node"]["nameKeyword"] == "campo"
    assert len(keywords["edges"]) == 1
    assert product.keywords.get().name_keyword == "campo"


UPDATE_PRODUCT_SLUG_MUTATION = """
    mutation($id: ID!, $slug: String, $category: ID!, $paymentMethods: [ID!]) {
        productUpdate(
            id: $id
            input: {
                slug: $slug
...
def test_update_product_slug_exists(
    staff_api_client, product, permission_manage_products, card_payment_method
):
    query = UPDATE_PRODUCT_SLUG_MUTATION
    input_slug = "test-slug"
...
def test_update_product_with_negative_weight(
    ...
    variables = {"productId": product_id, "weight": -1, "category": category_id}
...
def test_update_product_name(
    staff_api_client, permission_manage_products, card_payment_method, product, category
):
...
def test_update_product_slug_with_existing_value(
    staff_api_client, permission_manage_products, product, card_payment_method
):
    query = UPDATE_PRODUCT
    second_product = Product.objects.get(pk=product.pk)
    second_product.id = None
...
def test_delete_product_variant_in_draft_order(
    ...
    for variant in product.variants.all():
        ...
        net = variant.get_price(product, [], channel_EUR, variant_channel_listing, None)
...
def test_product_type(user_api_client, product_type, channel_EUR):
    query = """
    query ($channel: String){
...
def test_create_product_type_with_unicode_in_name(
    ...
    variables = {"name": name, "company": company_id, "category": category_id}
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_product_types_and_attributes]
    )
    content = get_graphql_content(response)
...
def test_product_variant_without_price_by_id_as_staff_with_permission(
    staff_api_client, variant, channel_EUR, permission_manage_products
):
    query = QUERY_PRODUCT_VARAINT_BY_ID
    variant.channel_listings.all().delete()
    variant.channel_listings.create(channel=channel_EUR)
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)

    variables = {"id": variant_id, "channel": channel_EUR.slug}
    response = staff_api_client.post_graphql(
        query,
        variables,
        permissions=[permission_manage_products],
...
def test_product_variant_without_price_by_id_as_staff_without_permission(
    staff_api_client, variant, channel_EUR
):
    query = QUERY_PRODUCT_VARAINT_BY_ID
    variant.channel_listings.all().delete()
    variant.channel_listings.create(channel=channel_EUR)
...
def test_product_variant_without_price_by_id_as_app_without_permission(
    app_api_client, variant, channel_EUR
):
    query = QUERY_PRODUCT_VARAINT_BY_ID
    variant.channel_listings.all().delete()
    variant.channel_listings.create(channel=channel_EUR)
...
def test_product_variant_without_price_by_id_as_user(
    user_api_client, variant, channel_EUR
):
    query = QUERY_PRODUCT_VARAINT_BY_ID
    variant.channel_listings.all().delete()
    variant.channel_listings.create(channel=channel_EUR)
...
def test_variant_query_invalid_id(user_api_client, variant, channel_EUR):
    variant_id = "'"
    variables = {
        "id": variant_id,
        "channel": channel_EUR.slug,
...
def test_variant_query_with_invalid_object_type(user_api_client, variant, channel_EUR):
...
def test_product_variant_without_price_by_sku_as_staff_with_permission(
    ...
    variables = {"sku": variant.sku, "channel": channel_EUR.slug}
    response = staff_api_client.post_graphql(
        query,
        variables,
        permissions=[permission_manage_products],
...
@pytest.mark.skip(
    reason="dRural can offer free products and users permissions have changed."
)
def test_product_variant_without_price_by_sku_as_app_with_permission(
    app_api_client, variant, channel_EUR, permission_manage_products
):
    query = """
        query getProductVariant($sku: String!, $channel: String) {
            productVariant(sku: $sku, channel: $channel) {
...
def test_product_variants_by_ids(staff_api_client, variant, channel_EUR):
    query = """
    ...
    variables = {"ids": [variant_id], "channel": channel_EUR.slug}
    response = staff_api_client.post_graphql(query, variables)
    content = get_graphql_content(response)
...
def test_product_variants_by_customer(user_api_client, variant, channel_EUR):
    query = """
        query getProductVariants($ids: [ID!], $channel: String) {
            productVariants(ids: $ids, first: 1, channel: $channel) {
                edges {
...
def test_product_variants_no_ids_list(user_api_client, variant, channel_EUR):
    query = """
...
def test_product_variant_price(
    ...
    product = variant.product
    ProductVariantChannelListing.objects.filter(
        channel=channel_EUR, variant__product_id=product.pk
...
def test_product_variant_without_price_as_user(
    ...
    variant.channel_listings.filter(channel=channel_EUR).update(price_amount=None)
    product_id = graphene.Node.to_global_id("Product", variant.product.id)
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.id)
    variables = {
        "id": product_id,
...
def test_product_variant_without_price_as_staff_with_permission(
    staff_api_client, variant, stock, channel_EUR, permission_manage_products
):
...
def test_product_variant_price_no_address(
    mock_get_variant_availability, user_api_client, variant, stock, channel_EUR
):
    channel_EUR.default_country = "FR"
    channel_EUR.save()
    product_id = graphene.Node.to_global_id("Product", variant.product.id)
    variables = {"id": product_id, "channel": channel_EUR.slug}
...
def test_report_product_sales_channel_EUR(
    staff_api_client,
    order_with_lines,
    permission_manage_products,
    permission_manage_orders,
...
def test_variant_digital_content(
    staff_api_client, permission_manage_products, digital_content, channel_EUR
):
    query = """
...
def test_collections_query_with_filter(
    ...
    CollectionChannelListing.objects.bulk_create(
        [
            CollectionChannelListing(
                channel=channel_EUR, collection=collection, is_published=published[num]
            )
...
def test_collections_query_with_sort(
    ...
    variables = {"sort_by": collection_sort, "channel": channel_EUR.slug}
    staff_api_client.user.user_permissions.add(permission_manage_products)
...
def test_update_or_create_variant_stocks(variant, warehouses):
    ...
    assert variant.stocks.all()[0].warehouse.pk == warehouses[0].pk
    assert variant.stocks.all()[1].warehouse.pk == warehouses[1].pk
    assert {stock.quantity for stock in variant.stocks.all()} == {
        data["quantity"] for data in stocks_data
    }
...
def test_hidden_product_access_with_permission_manage_channels(
    ...
    content = get_graphql_content(response)
    total_count = content["data"]["products"]["totalCount"]
    # Staff user with permissions can get all products
    assert total_count == 3


def test_query_product_for_federation(api_client, product, channel_EUR):
    product_id = graphene.Node.to_global_id("Product", product.pk)
    variables = {
        "representations": [
            {
...
def test_query_product_media_for_federation(
    api_client, product_with_image, channel_EUR
):
    media = product_with_image.media.first()
...
def test_query_product_type_for_federation(api_client, product, channel_EUR):
    product_type = product.product_type
    product_type_id = graphene.Node.to_global_id("ProductType", product_type.pk)
    variables = {
        "representations": [
