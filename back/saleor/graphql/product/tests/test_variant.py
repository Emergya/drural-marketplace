...
def test_fetch_variant(
    ...
    variables = {"id": variant_id, "countryCode": "EU", "channel": channel_EUR.slug}
...
def test_get_product_variant_stocks(
    ...
    pl_stocks = variant.stocks.filter(
        warehouse__shipping_zones__countries__contains="ES"
    )
    data = content["data"]["productVariant"]
...
@pytest.mark.skip(reason="We will not alloe creation of variants.")
...
def test_fetch_all_variants_customer(
    user_api_client, unavailable_product_with_variant, channel_EUR
):
    data = _fetch_all_variants(user_api_client, variables={"channel": channel_EUR.slug})
    assert data["totalCount"] == 0


def test_fetch_all_variants_anonymous_user(
    api_client, unavailable_product_with_variant, channel_EUR
):
    data = _fetch_all_variants(api_client, variables={"channel": channel_EUR.slug})
    assert data["totalCount"] == 0
...
def test_product_variants_by_ids(user_api_client, variant, channel_EUR):
...
def test_product_variants_by_invalid_ids(user_api_client, variant, channel_EUR):
    query = QUERY_PRODUCT_VARIANTS_BY_IDS
    variant_id = "cbs"
...
def test_product_variants_visible_in_listings_by_staff_without_manage_products(
    ...
    # when
    data = _fetch_all_variants(
        staff_api_client, variables={"channel": channel_EUR.slug}
    )
...
def test_product_variants_visible_in_listings_by_app_without_manage_products(
    ...
    # when
    data = _fetch_all_variants(app_api_client, variables={"channel": channel_EUR.slug})

    assert data["totalCount"] == product_count - 1


def test_product_variants_visible_in_listings_by_app_with_perm(
    app_api_client, product_list, permission_manage_products, channel_EUR
):
    # given
    product_list[0].channel_listings.all().update(visible_in_listings=False)
...
def test_fetch_unpublished_variant_customer(
    ...
    variant = unavailable_product_with_variant.variants.first()
    data = _fetch_variant(user_api_client, variant, channel_slug=channel_EUR.slug)
    assert data is None
...
def test_fetch_unpublished_variant_anonymous_user(
    ...
    variant = unavailable_product_with_variant.variants.first()
    data = _fetch_variant(api_client, variant, channel_slug=channel_EUR.slug)
    assert data is None
            ...
            "channelListings": [
                {
                    "price": 10.0,
                    "costPrice": 11.0,
                    "channelId": graphene.Node.to_global_id("Channel", channel_EUR.pk),
                }
            ],
            "attributes": [{"id": size_attribute_id, "values": [attribute_value.name]}],
        },
...
def test_product_variant_bulk_create_product_not_assigned_to_channel(
    ...
    assert not ProductChannelListing.objects.filter(
        product=product, channel=channel_EUR
    ).exists()
    size_attribute_id = graphene.Node.to_global_id("Attribute", size_attribute.pk)
    attribute_value = size_attribute.values.last()
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.pk)
...
def test_query_product_variant_for_federation(api_client, variant, channel_EUR):
    variant_id = graphene.Node.to_global_id("ProductVariant", variant.pk)
    variables = {
        "representations": [
            {
