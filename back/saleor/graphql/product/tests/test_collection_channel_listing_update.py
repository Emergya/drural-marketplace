...
def test_collection_channel_listing_update_duplicated_ids_in_add_and_remove(
    ...
    response = staff_api_client.post_graphql(
        COLLECTION_CHANNEL_LISTING_UPDATE_MUTATION,
        variables=variables,
        permissions=(permission_manage_collection, permission_manage_products),
    )
...
def test_collection_channel_listing_update_duplicated_channel_in_add(
    ...
    response = staff_api_client.post_graphql(
        COLLECTION_CHANNEL_LISTING_UPDATE_MUTATION,
        variables=variables,
        permissions=(permission_manage_products, permission_manage_collection),
    )
    content = get_graphql_content(response)
...
def test_collection_channel_listing_update_as_staff_user(
    ...
    publication_date_eur = collection_channel_listing.publication_date.isoformat()
    assert not data["errors"]
    assert collection_data["slug"] == published_collection.slug
    assert collection_data["channelListings"][0]["isPublished"] is False
    assert (
        collection_data["channelListings"][0]["publicationDate"] == publication_date_eur
    )
    assert collection_data["channelListings"][0]["channel"]["slug"] == channel_EUR.slug


@pytest.mark.skip(reason="Only staff users can manage collections")
def test_collection_channel_listing_update_as_app(
    ...
    assert collection_data["channelListings"][1]["channel"]["slug"] == channel_EUR.slug


def test_collection_channel_listing_update_as_customer(
    user_api_client, collection, channel_EUR
):
    # given
    collection_id = graphene.Node.to_global_id("Collection", collection.pk)
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
...
def test_collection_channel_listing_update_as_anonymous(
    api_client, collection, channel_EUR
):
    # given
    collection_id = graphene.Node.to_global_id("Collection", collection.pk)
    channel_id = graphene.Node.to_global_id("Channel", channel_EUR.id)
