...
def test_fetch_all_variants(api_client, product_variant_list, channel_EUR):
    with warnings.catch_warnings(record=True) as warns:
        response = api_client.post_graphql(QUERY_FETCH_ALL_VARIANTS)

    content = get_graphql_content(response)
    data = content["data"]["productVariants"]
    # A new variante has been added in the fixture
    assert data["totalCount"] == len(product_variant_list) + 1
    assert any(
        [str(warning.message) == DEPRECATION_WARNING_MESSAGE for warning in warns]
    )
