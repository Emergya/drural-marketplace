...
def test_collection_query_by_id(user_api_client, published_collection, channel_EUR):
    variables = {
...
@pytest.mark.skip(
    reason="You can only get information of unpublished products if you are staff"
)
def test_collection_query_unpublished_collection_by_id_as_app(
    app_api_client, unpublished_collection, permission_manage_products, channel_EUR
):
    # given
    variables = {
...
def test_collection_query_by_slug(user_api_client, published_collection, channel_EUR):
    variables = {
...
def test_collection_query_unpublished_collection_by_slug_as_staff(
    staff_api_client, unpublished_collection, permission_manage_products, channel_EUR
):
    # given
    user = staff_api_client.user
    user.user_permissions.add(permission_manage_products)

    variables = {"slug": unpublished_collection.slug, "channel": channel_EUR.slug}
...
def test_collection_query_unpublished_collection_by_slug_and_anonymous_user(
    api_client, unpublished_collection, channel_EUR
):
    # given
    variables = {"slug": unpublished_collection.slug, "channel": channel_EUR.slug}
...
def test_filter_collection_products(
    user_api_client, product_list, published_collection, channel_EUR
):
    # given
...
def test_create_collection(
    ...
    response = staff_api_client.post_multipart(
        body, permissions=[permission_manage_collection]
    )
    content = get_graphql_content(response)
...
def test_create_collection_trigger_product_update_webhook(
    ...
    response = staff_api_client.post_graphql(
        query, variables, permissions=[permission_manage_collection]
    )
    content = get_graphql_content(response)
    data = content["data"]["collectionCreate"]["collection"]
...
def test_create_collection_without_background_image(
    monkeypatch, staff_api_client, product_list, permission_manage_collection
):
    query = CREATE_COLLECTION_MUTATION
...
def test_create_collection_name_with_unicode(
    staff_api_client, permission_manage_collection, channel_EUR
):
    query = CREATE_COLLECTION_MUTATION
    name = "わたし わ にっぽん です"
    variables = {"name": name}
...
def test_update_collection(
    monkeypatch, staff_api_client, collection, permission_manage_collection
):
    query = """
...
def test_update_collection_with_background_image(
    monkeypatch, staff_api_client, collection, permission_manage_collection, media_root
):
    mock_create_thumbnails = Mock(return_value=None)
    monkeypatch.setattr(
        (
...
def test_update_collection_invalid_background_image(
    staff_api_client, collection, permission_manage_collection
):
...
def test_add_products_to_collection(
    staff_api_client, collection, product_list, permission_manage_collection
):
    query = """
...
def test_collections_query_ids_not_exists(
    user_api_client, published_collection, channel_EUR
):
...
def test_collection_image_query(
    user_api_client, published_collection, media_root, channel_EUR
):
...
def test_update_collection_mutation_remove_background_image(
    staff_api_client, collection_with_image, permission_manage_collection
):
    query = """
...
def test_fetch_unpublished_collection_customer(
    user_api_client, unpublished_collection, channel_EUR
):
    collection_data = _fetch_collection(
        user_api_client, unpublished_collection, channel_EUR.slug
    )
    assert collection_data is None
...
def test_fetch_unpublished_collection_anonymous_user(
    ...
    collection_data = _fetch_collection(
        api_client, unpublished_collection, channel_EUR.slug
    )
    assert collection_data is None
...
def test_sort_collection_products_by_name(
    staff_api_client, published_collection, product_list, channel_EUR
):
    # given
    for product in product_list:
        published_collection.products.add(product)
...
def test_query_collection_for_federation(api_client, published_collection, channel_EUR):
    collection_id = graphene.Node.to_global_id("Collection", published_collection.pk)
    variables = {
        "representations": [
            {
