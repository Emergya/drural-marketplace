...
def test_category_delete_updates_discounted_price(
    ...
    parent_product = parent.children.first().products.first()
    parent_product_count = parent.children.first().products.first().categories.count()
    child_product = parent.products.first()
    child_product_count = parent.products.first().categories.count()
    ...
    assert len(parent_product.categories.all()) == parent_product_count - 1
    child_product.refresh_from_db()
    assert len(child_product.categories.all()) == child_product_count - 1
