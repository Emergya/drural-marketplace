...
from ..utils.filters import (
    ...
    filter_range_field,
    filter_by_query_param_and_keywords,
)
...
def filter_products_by_closeness(qs, channel_slug, radius, latitude, longitude):
    channels = Channel.objects.filter(slug=channel_slug).values("pk")
    product_channel_listings = ProductChannelListing.objects.filter(
        Exists(channels.filter(pk=OuterRef("channel_id")))
    )

    my_current_location = Point(latitude, longitude, srid=4326)
    qs = qs.filter(
        address__location_point__distance_lte=(my_current_location, D(km=radius))
    ).annotate(distance=Distance("address__location_point", my_current_location))
    return qs.filter(Exists(product_channel_listings.filter(product_id=OuterRef("pk"))))


def filter_products_by_consumption(qs, start_date, end_date):
    # This filter gives us a QS which contains prodoucts consumed in a date range
    orders = Order.objects.filter(created__range=[start_date, end_date]).values("pk")
    order_lines = OrderLine.objects.filter(
        Exists(orders.filter(pk=OuterRef("order_id")))
    )
    variants = ProductVariant.objects.filter(
        Exists(order_lines.filter(variant_id=OuterRef("id")))
    )
    return qs.filter(Exists(variants.filter(product_id=OuterRef("pk"))))
...
def filter_products_by_categories(qs, category_pks):
    categories = Category.objects.filter(id__in=category_pks)
    categories = Category.tree.get_queryset_descendants(categories, include_self=True)
    return qs.filter(categories__in=categories).distinct()

def filter_products_by_company(qs, company_pk):
    company = Company.objects.filter(id=company_pk).first()
...
def filter_company(qs, _, value):
    if value:
        _, company_pk = from_global_id_or_error(
            value, product_types.CompanyType
        )
        qs = filter_products_by_company(qs, company_pk)
    return qs
...
def filter_has_category(qs, _, value):
    return qs.filter(categories__isnull=not value)
...
def _filter_review_rating(qs, value, stars):
    if value:
        qs = qs.filter(rating=stars)
    return qs
...
def _filter_products_created_date(qs, _, value, channel_slug):
    channel = Channel.objects.filter(slug=channel_slug).values("pk")
    product_channel_listings = ProductChannelListing.objects.filter(
        Exists(channel.filter(pk=OuterRef("channel_id")))
    ).values("product_id")
...
def _filter_products_modified_date(qs, _, value, channel_slug):
    channel = Channel.objects.filter(slug=channel_slug).values("pk")
    product_channel_listings = ProductChannelListing.objects.filter(
        Exists(channel.filter(pk=OuterRef("channel_id")))
    ).values("product_id")
...
def _filter_has_review_in_range(qs, _, value):
    return qs.filter(
        ratings__created_at__date__range=(
            value.get("gte").date(),
            value.get("lte").date(),
        )
    ).annotate(stars=Avg("ratings__rating"))
...
def _filter_closeness(qs, _, value, channel_slug):
    if value:
        qs = filter_products_by_closeness(
            qs,
            channel_slug,
            radius=value["radius"],
...
def _filter_consumption(qs, _, value):
    if value:
        qs = filter_products_by_consumption(
            qs,
...
def filter_search(qs, _, value):
    search_fields = ("name", "description_plaintext", "variants__sku")
    if value:
        qs = filter_by_query_param_and_keywords(qs, value.strip(), search_fields)
    return qs
...
class ProductFilter(MetadataFilterBase):
    ...
    closeness = ObjectTypeFilter(
        input_class=ClosenessRangeInput, method="filter_closeness"
    )
    consumption = ObjectTypeFilter(
        input_class=DateTimeRangeInput, method="filter_consumption"
    )
    ...
    created_date = ObjectTypeFilter(
        input_class=DateTimeRangeInput, method="filter_created_date"
    )
    modified_date = ObjectTypeFilter(
        input_class=DateTimeRangeInput, method="filter_modified_date"
    )
    has_review_in_range = ObjectTypeFilter(
        input_class=DateTimeRangeInput, method="filter_has_review_in_range"
    )
    ...
    def filter_closeness(self, queryset, name, value):
        channel_slug = get_channel_slug_from_filter_data(self.data)
        return _filter_closeness(queryset, name, value, channel_slug)

    def filter_consumption(self, queryset, name, value):
        return _filter_consumption(queryset, name, value)
    ...
    def filter_created_date(self, queryset, name, value):
        channel_slug = get_channel_slug_from_filter_data(self.data)
        return _filter_products_created_date(
            queryset,
            name,
            value,
            channel_slug,
    ...
    def filter_modified_date(self, queryset, name, value):
        channel_slug = get_channel_slug_from_filter_data(self.data)
        return _filter_products_modified_date(
            queryset,
            name,
            value,
            channel_slug,
    ...
    def filter_has_review_in_range(self, queryset, name, value):
        return _filter_has_review_in_range(queryset, name, value)
...
class ProductRatingFilter(MetadataFilterBase):
...
class BookableResourceFilter(django_filters.FilterSet):
    search = django_filters.CharFilter(method="bookable_resource_filter_search")
    is_active = django_filters.BooleanFilter(
        method="bookable_resource_filter_is_active"
    )
    ...
    @classmethod
    def bookable_resource_filter_search(cls, queryset, _name, value):
        if value:
            return queryset.filter(name__icontains=value)
        return queryset

    @classmethod
    def bookable_resource_filter_is_active(cls, queryset, _name, value):
        if value is not None:
            return queryset.filter(is_active=value)
        return queryset


class BookableResourceFilterInput(FilterInputObjectType):
    class Meta:
        filterset_class = BookableResourceFilter


class ProductRatingFilterInput(FilterInputObjectType):
    class Meta:
