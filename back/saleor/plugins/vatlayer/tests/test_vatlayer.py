...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            Money(10, "EUR"),
            True,
            TaxedMoney(net=Money("8.13", "EUR"), gross=Money(10, "EUR")),
        ),
    ],
)
...
def test_apply_tax_to_price_do_not_include_tax(site_settings, taxes):
    ...
    assert apply_tax_to_price(taxes, "standard", money) == TaxedMoney(
        net=Money(100, "EUR"), gross=Money(123, "EUR")
    )
    assert apply_tax_to_price(taxes, "medical", money) == TaxedMoney(
        net=Money(100, "EUR"), gross=Money(108, "EUR")
    )

    taxed_money = TaxedMoney(net=Money(100, "EUR"), gross=Money(100, "EUR"))
...
def test_apply_tax_to_price_do_not_include_tax_fallback_to_standard_rate(
    ...
    money = Money(100, "EUR")
    taxed_money = TaxedMoney(net=Money(100, "EUR"), gross=Money(123, "EUR"))
...
def test_apply_tax_to_price_include_tax(taxes):
    money = Money(100, "EUR")
    assert apply_tax_to_price(taxes, "standard", money) == TaxedMoney(
        net=Money("81.30", "EUR"), gross=Money(100, "EUR")
    )
    assert apply_tax_to_price(taxes, "medical", money) == TaxedMoney(
        net=Money("92.59", "EUR"), gross=Money(100, "EUR")
    )
...
def test_apply_tax_to_price_no_taxes_return_taxed_money_range():
    money_range = MoneyRange(Money(100, "EUR"), Money(200, "EUR"))
    taxed_money_range = TaxedMoneyRange(
        TaxedMoney(net=Money(100, "EUR"), gross=Money(100, "EUR")),
        TaxedMoney(net=Money(200, "EUR"), gross=Money(200, "EUR")),
    )
...
def test_vatlayer_plugin_caches_taxes(
    vatlayer, monkeypatch, product, address, channel_EUR
):
    ...
    plugin = manager.get_plugin(VatlayerPlugin.PLUGIN_ID, channel_slug=channel_EUR.slug)
    variant = product.variants.first()
    channel_listing = variant.channel_listings.get(channel=channel_EUR)
    price = variant.get_price(
        product, [], channel_EUR, channel_listing, product.categories.all(), None
    )
...
def test_calculate_checkout_total_from_origin_country(
    ...
    # make sure that we applied DE taxes (16%)
    assert total == TaxedMoney(net=Money("21.00", "EUR"), gross=Money("25.00", "EUR"))
...
def test_calculate_checkout_total_with_excluded_country(
    ...
    assert total == TaxedMoney(net=Money("25.00", "EUR"), gross=Money("25.00", "EUR"))
...
def test_calculate_checkout_subtotal_from_origin_country(
    ...
    assert total == TaxedMoney(net=Money("12.60", "EUR"), gross=Money("15.00", "EUR"))
...
def test_calculate_checkout_subtotal_with_excluded_country(
    ...
    plugin = vatlayer_plugin(origin_country="DE", excluded_countries="PL")
    manager = get_plugins_manager()
    ...
    # make sure that we not have VAT
    assert total == TaxedMoney(net=Money("15.00", "EUR"), gross=Money("15.00", "EUR"))
...
def test_calculate_order_shipping_from_origin_country(
    ...
    # make sure that we applied DE taxes (19%)
    assert price == TaxedMoney(net=Money("8.40", "EUR"), gross=Money("10.00", "EUR"))
...
def test_calculate_order_shipping_with_excluded_country(
    ...
    assert price == TaxedMoney(net=Money("10.00", "EUR"), gross=Money("10.00", "EUR"))
...
def test_calculate_order_line_unit_from_origin_country(
    ...
    # make sure that we applied DE taxes (19%)
    assert line_price == TaxedMoney(
        net=Money("8.40", "EUR"), gross=Money("10.00", "EUR")
    )
...
def test_calculate_checkout_line_total(
    ...
    assert line_price == TaxedMoney(
        net=Money("8.13", "EUR") * line.quantity,
        gross=Money("10.00", "EUR") * line.quantity,
    )
...
def test_calculate_checkout_line_total_from_origin_country(
    ...
    # make sure that we applied DE taxes (19%)
    assert line_price == TaxedMoney(
        net=Money("8.40", "EUR") * line.quantity,
        gross=Money("10.00", "EUR") * line.quantity,
    )
...
def test_calculate_checkout_line_total_with_excluded_country(
    ...
    assert line_price == TaxedMoney(
        net=Money("10.00", "EUR") * line.quantity,
        gross=Money("10.00", "EUR") * line.quantity,
    )
...
def test_calculate_checkout_line_unit_price(
    ...
    manager = get_plugins_manager()
    total_price = TaxedMoney(net=Money("10.00", "EUR"), gross=Money("10.00", "EUR"))
...
def test_save_plugin_configuration(vatlayer, settings, channel_EUR):
    settings.PLUGINS = ["saleor.plugins.vatlayer.plugin.VatlayerPlugin"]
    manager = get_plugins_manager()
    manager.save_plugin_configuration(
        VatlayerPlugin.PLUGIN_ID, channel_EUR.slug, {"active": False}
    )
...
def test_apply_taxes_to_product(
    vatlayer, settings, variant, discount_info, channel_EUR
):
    settings.PLUGINS = ["saleor.plugins.vatlayer.plugin.VatlayerPlugin"]
    country = Country("PL")
    manager = get_plugins_manager()
    ...
    assert price == TaxedMoney(net=Money("4.07", "EUR"), gross=Money("5.00", "EUR"))


def test_apply_taxes_to_product_from_origin_country(
    vatlayer_plugin, settings, variant, discount_info, channel_EUR
):
    vatlayer_plugin(
        origin_country="DE", countries_to_calculate_taxes_from_origin="PL,FR"
    )
    ...
    assert price == TaxedMoney(net=Money("4.20", "EUR"), gross=Money("5.00", "EUR"))
...
def test_apply_taxes_to_product_with_excluded_country(
    ...
    assert price == TaxedMoney(net=Money("5.00", "EUR"), gross=Money("5.00", "EUR"))
...
def test_calculations_checkout_total_with_vatlayer(
    ...
    assert checkout_subtotal == TaxedMoney(
        net=Money("30", "EUR"), gross=Money("30", "EUR")
    )
...
def test_calculations_checkout_shipping_price_with_vatlayer(
    ...
    assert checkout_shipping_price == TaxedMoney(
        net=Money("0", "EUR"), gross=Money("0", "EUR")
    )


def test_skip_diabled_plugin(settings, channel_EUR):
    settings.PLUGINS = ["saleor.plugins.vatlayer.plugin.VatlayerPlugin"]
    manager = get_plugins_manager()
    plugin: VatlayerPlugin = manager.get_plugin(
        VatlayerPlugin.PLUGIN_ID, channel_EUR.slug
    )
...
def test_get_checkout_line_tax_rate(
    ...
    unit_price = TaxedMoney(Money(12, "EUR"), Money(15, "EUR"))
...
def test_get_checkout_line_tax_rate_with_excluded_country(
    ...
    unit_price = TaxedMoney(Money(15, "EUR"), Money(15, "EUR"))
...
def test_get_order_shipping_tax_rate(
    ...
    shipping_price = TaxedMoney(Money(12, "EUR"), Money(15, "EUR"))
...
def test_get_order_shipping_tax_rate_with_excluded_country(
    ...
    shipping_price = TaxedMoney(Money(15, "EUR"), Money(15, "EUR"))
