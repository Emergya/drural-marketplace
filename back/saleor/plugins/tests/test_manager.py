...
def test_manager_with_default_configuration_for_channel_plugins(settings, channel_EUR):
    settings.PLUGINS = [
    ...
    assert {channel_EUR.slug, channel_EUR.slug} == set(
...
def test_manager_with_channel_plugins(
    settings, channel_EUR, channel_plugin_configurations
):
    settings.PLUGINS = [
        "saleor.plugins.tests.sample_plugins.ChannelPluginSample",
    ]
    ...
    assert {channel_EUR.slug} == set(manager.plugins_per_channel.keys())
...
def test_manager_get_plugins_with_channel_slug(
    settings, channel_EUR, plugin_configuration, inactive_plugin_configuration
):
    settings.PLUGINS = [
    ...
    plugins = manager.get_plugins(channel_slug=channel_EUR.slug)

    assert plugins == manager.plugins_per_channel[channel_EUR.slug]
...
def test_manager_get_active_plugins_with_channel_slug(
    ...
    plugins = manager.get_plugins(channel_slug=channel_EUR.slug, active_only=True)
...
@pytest.mark.parametrize(
    ...
    [(["saleor.plugins.tests.sample_plugins.PluginSample"], "1.0"), ([], "30.0")],
)
...
@pytest.mark.parametrize(
    ...
    [
        (TaxedMoney(Money(12, "EUR"), Money(15, "EUR")), Decimal("0.25")),
...
def test_manager_get_checkout_shipping_tax_rate_sample_plugin(
    ...
    plugins = ["saleor.plugins.tests.sample_plugins.PluginSample"]
    shipping_price = TaxedMoney(Money(12, "EUR"), Money(14, "EUR"))
...
@pytest.mark.parametrize(
    ...
    [
        (TaxedMoney(Money(12, "EUR"), Money(14, "EUR")), Decimal("0.1667")),
        (Decimal("0.0"), Decimal("0.0")),
    ],
)
...
@pytest.mark.parametrize(
    ...
    [
        (
            ["saleor.plugins.tests.sample_plugins.PluginSample"],
            TaxedMoney(
                net=Money(amount=10, currency="EUR"),
                gross=Money(amount=10, currency="EUR"),
            ),
            1,
        ),
        (
            [],
            TaxedMoney(
                net=Money(amount=15, currency="EUR"),
                gross=Money(amount=15, currency="EUR"),
            ),
            2,
        ),
    ],
...
def test_manager_apply_taxes_to_product(product, plugins, price, channel_EUR):
    country = Country("PL")
...
def test_manager_apply_taxes_to_shipping(
    shipping_method, address, plugins, price_amount, channel_EUR
):
    shipping_price = shipping_method.channel_listings.get(
        channel_id=channel_EUR.id
    ).price
    expected_price = Money(price_amount, "EUR")
    taxed_price = PluginsManager(plugins=plugins).apply_taxes_to_shipping(
        shipping_price, address, channel_slug=channel_EUR.slug
    )
    assert TaxedMoney(expected_price, expected_price) == taxed_price
...
def test_manager_serve_list_of_payment_gateways(channel_EUR):
...
def test_manager_serve_list_all_payment_gateways(channel_EUR):
    expected_gateways = [
        PaymentGateway(
            id=ActivePaymentGateway.PLUGIN_ID,
            name=ActivePaymentGateway.PLUGIN_NAME,
...
def test_manager_serve_list_all_payment_gateways_specified_currency(channel_EUR):
    expected_gateways = [
        PaymentGateway(
            id=ActivePaymentGateway.PLUGIN_ID,
            name=ActivePaymentGateway.PLUGIN_NAME,
...
def test_list_external_authentications(channel_EUR):
    plugins = [
        "saleor.plugins.tests.sample_plugins.PluginInactive",
        "saleor.plugins.tests.sample_plugins.ActivePaymentGateway",
        "saleor.plugins.tests.sample_plugins.PluginSample",
...
def test_list_external_authentications_active_only(channel_EUR):
    plugins = [
        "saleor.plugins.tests.sample_plugins.PluginInactive",
        "saleor.plugins.tests.sample_plugins.ActivePaymentGateway",
        "saleor.plugins.tests.sample_plugins.PluginSample",
...
def test_run_method_on_plugins_default_value_when_not_existing_method_is_called(
    channel_EUR, all_plugins_manager
):
    default_value = "default"
...
def test_run_method_on_plugins_only_on_active_ones(
    mocked_method, channel_EUR, all_plugins_manager
):
...
def test_run_check_payment_balance(channel_EUR):
...
def test_run_check_payment_balance_not_implemented(channel_EUR):
