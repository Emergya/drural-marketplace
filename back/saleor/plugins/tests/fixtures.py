...
@pytest.fixture
def channel_plugin_configurations(db, channel_EUR):
    eur_configuration = copy.deepcopy(ChannelPluginSample.DEFAULT_CONFIGURATION)
    eur_configuration[0]["value"] = channel_EUR.slug
    ...
    return PluginConfiguration.objects.bulk_create(
        [
            PluginConfiguration(
                ...
                configuration=eur_configuration,
            ),
            # PluginConfiguration(
            #     identifier=ChannelPluginSample.PLUGIN_ID,
            #     channel=channel_EUR,
