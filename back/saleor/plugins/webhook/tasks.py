...
def _get_webhooks_for_event(event_type, data, webhooks=None):
    ...
    if data:
        ...
        #   Webhooks will be active for events of your own company and staff
        ...
        if not isinstance(dict, list):
            if event_type == WebhookEventType.NOTIFY_USER:
                if dict.get("notify_event") not in supported_event:
                    return []
                if "company" in dict["payload"]:
                    company_pk = dict["payload"]["company"]["id"]
                    company = Company.objects.get(pk=company_pk)
                    users = staff_users | company.managers.all()
                    webhooks = webhooks.filter(app__user__in=users)
                elif recipient_email := dict["payload"]["recipient_email"]:
                    user = User.objects.filter(email=recipient_email)
                    users = staff_users | user
                    webhooks = webhooks.filter(app__user__in=users)
                elif isinstance(dict["payload"]["order"]["id"], int):
                    order_pk = dict["payload"]["order"]["id"]
                else:
                    _, order_pk = from_global_id_or_error(
                        dict["payload"]["order"]["id"], only_type="Order"
                    )
            elif "order_id" in dict:
                order_pk = dict["order_id"]
            elif "id" in dict:
                _, order_pk = from_global_id_or_error(dict["id"], only_type="Order")

            if "order_pk" in locals():
                order = Order.objects.get(pk=order_pk)
                companies = set(
                    [line.variant.product.company for line in order.lines.all()]
                )
                managers = list(companies)[0].managers.all()

                for company in companies:
                    managers = managers & company.managers.all()

                users = staff_users | managers
                webhooks = webhooks.filter(app__user__in=users)

        else:
            for order in dict:
                if "type" in order and order["type"] == "Order" and order["lines"]:
                    _, order_pk = from_global_id_or_error(
                        order["id"], only_type="Order"
                    )
                    order = Order.objects.get(pk=order_pk)
                    companies = set(
                        [line.variant.product.company for line in order.lines.all()]
...
def trigger_webhooks_for_event(event_type, data):
    ...
    webhooks = _get_webhooks_for_event(event_type, data)
...
def trigger_webhook_sync(event_type: str, data: str, app: "App"):
    ...
    webhooks = _get_webhooks_for_event(event_type, data, app.webhooks.all())
