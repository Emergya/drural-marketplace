...
@pytest.mark.skip(reason="the webhook trigger is not receiving data")
...
@mock.patch("saleor.plugins.webhook.plugin.trigger_webhooks_for_event.delay")
def test_notify_user(mocked_webhook_trigger, settings, customer_user, channel_EUR):
    settings.PLUGINS = ["saleor.plugins.webhook.plugin.WebhookPlugin"]
    manager = get_plugins_manager()

    redirect_url = "http://redirect.com/"
    send_account_confirmation(customer_user, redirect_url, manager, channel_EUR.slug)
    ...
    payload = {
        ...
        "user_name": customer_user.first_name,
        "language_code": "en",
        **get_site_context(),
    }
