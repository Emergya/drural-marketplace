...
def test_get_payment_gateways(
    ...
    response_data = plugin.get_payment_gateways("EUR", None, None)
...
def test_get_payment_gateways_filters_out_unsupported_currencies(
    ...
    mock_send_request.return_value = mock_json_response
    response_data = plugin.get_payment_gateways("DUM", None, None)
    mock_json_response[0].id = response_data[0].id[:6] + mock_json_response[0].id
    assert response_data == mock_json_response
...
def test_get_payment_gateways_for_checkout(
    ...
    plugin.get_payment_gateways("EUR", checkout, None)
