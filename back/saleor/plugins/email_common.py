...
# IMPORTANT: keep inline styles due to gmail class attribute bloking
# PARTIALS
GLOBAL_STYLES_PARTIAL = """
    <!-- dRural global styles -->
    <style type="text/css">
        p {
            margin: 0;
        }
...
BODY_PARTIAL = """
    <body style="box-sizing: border-box; color: #000; font-family: 'Poppins', sans-serif; font-size: 16px; font-weight: 400; line-height: 24px;">
"""
...
EMAIL_CONTAINER_PARTIAL = """
    <div style="background: #F7F6F8; padding-top: 32px; padding-bottom: 24px;">
"""

EMAIL_WRAPPER_PARTIAL = """
    <div style="background: #fff; box-shadow: 0px 4px 16px rgb(0 0 0 / 10%); margin: 32px auto 24px; width: 80%;">
"""

CONTENT_WRAPPER_PARTIAL = """
...
LOGO_PARTIAL = """
    <div style="text-align: center;">
        <img style="max-height: 64px;" src="{{ site_logo }}"/>
    </div>
"""

IMAGE_PARTIAL = """
    {{#if email_image }}
        <div style="align-items: center; display: flex; justify-content: center; overflow: hidden;">
            <img style="width: 85%; padding-top: 32px; margin: auto;" src="{{ email_image }}"/>
        </div>
    {{/if}}
"""

TITLE_PARTIAL = """
    <h3 style="font-size: 22px; font-weight: 600; line-height: 32px; margin: 0; margin-bottom: 22px;">
        {{#if user.first_name }}
            Hi, {{ user.first_name }}!
        {{else}}
            Hi!
...
FOOTER_PARTIAL = """
    <div style="padding: 0 16px; font-size: 14px; line-height: 24px; opacity: 0.6; text-align: center;">
        <p>© 2022 {{ site_name }}</p>
        <p>Designed and developed by dRural.</p>
        <img style="margin-top: 8px; max-height: 40px;" src="{{ site_icon_logo }}"/>
    </div>
"""

SCHEMA_MARKUP_PARTIAL = """
    <!-- Script -->
...
AUTOMATICALLY_GENERATED_PARTIAL = """
...
def send_email(
    ...
    # Add partials
    global_styles = compiler.compile(GLOBAL_STYLES_PARTIAL)
    fonts = compiler.compile(FONTS_PARTIAL)
    body = compiler.compile(BODY_PARTIAL)
    close_body = compiler.compile(CLOSE_BODY_PARTIAL)
    email_container = compiler.compile(EMAIL_CONTAINER_PARTIAL)
    email_wrapper = compiler.compile(EMAIL_WRAPPER_PARTIAL)
    content_wrapper = compiler.compile(CONTENT_WRAPPER_PARTIAL)
    close_div = compiler.compile(CLOSE_DIV_PARTIAL)
    logo = compiler.compile(LOGO_PARTIAL)
    image = compiler.compile(IMAGE_PARTIAL)
    title = compiler.compile(TITLE_PARTIAL)
    schema_markup = compiler.compile(SCHEMA_MARKUP_PARTIAL)
    footer = compiler.compile(FOOTER_PARTIAL)
    partials = {
    ...
    message = template(context, helpers=helpers, partials=partials)
...
def get_email_template_or_default(
    ...
    # Necesito averguar si se envia el plugin configurado.
    ...
    if email_template_str == DEFAULT_EMAIL_VALUE:
        email_template_str = get_default_email_template(
            default_template_file_name, default_template_path, language_code
        )
...
def get_default_email_template(
    template_file_name: str, default_template_path: str, language_code: str = "en"
) -> str:
    """Get default template."""
    default_template_path = os.path.join(default_template_path + f"/{language_code}/", template_file_name)
