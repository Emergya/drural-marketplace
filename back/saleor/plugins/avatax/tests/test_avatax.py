...
def test_calculate_order_line_total(
    ...
    total = quantize_price(total, total.currency)
    assert total == TaxedMoney(net=Money("30.00", "EUR"), gross=Money("36.90", "EUR"))
...
def test_calculate_order_line_total_order_not_valid(
    ...
    total = quantize_price(total, total.currency)
    assert total == TaxedMoney(net=Money("0.00", "EUR"), gross=Money("0.00", "EUR"))
...
def test_calculate_checkout_total_not_charged_product_and_shipping_with_0_price(
    ...
    shipping_channel_listing.price = Money(0, "EUR")
    ...
    assert total == TaxedMoney(
        net=Money(expected_amount, "EUR"), gross=Money(expected_amount, "EUR")
    )
...
def test_calculate_checkout_subtotal_for_product_without_tax(
    ...
    assert total == TaxedMoney(
        net=Money(expected_total, "EUR"), gross=Money(expected_total, "EUR")
    )
...
def test_calculate_checkout_line_unit_price(
    ...
    total_price = TaxedMoney(
        net=Money("10.00", "EUR") * 3, gross=Money("10.00", "EUR") * 3
    )
...
def test_get_checkout_line_tax_rate(
    ...
    unit_price = TaxedMoney(Money(12, "EUR"), Money(14, "EUR"))
...
def test_get_checkout_line_tax_rate_for_product_with_charge_taxes_set_to_false(
    ...
    unit_price = TaxedMoney(Money(12, "EUR"), Money(12, "EUR"))
...
def test_get_order_line_tax_rate(
    ...
    # then
    assert tax_rate == Decimal("0.1667")
...
def test__get_shipping_tax_rate_handles_multiple_tax_districts(
    avalara_response_for_checkout_with_items_and_shipping, channel_EUR
):
    manager = get_plugins_manager()
...
def test_get_plugin_configuration(settings, channel_EUR):
...
def test_save_plugin_configuration(
    api_get_request_mock, settings, channel_EUR, plugin_configuration
):
    settings.PLUGINS = ["saleor.plugins.avatax.plugin.AvataxPlugin"]
    plugin_configuration()
    ...
    manager.save_plugin_configuration(
        AvataxPlugin.PLUGIN_ID, channel_EUR.slug, {"active": True}
    )
...
def test_save_plugin_configuration_cannot_be_enabled_without_config(
    settings, plugin_configuration, channel_EUR
):
