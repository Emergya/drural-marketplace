...
def test_send_email_request_change(
    mocked_notify, site_settings, customer_user, channel_EUR
):
...
def test_send_email_changed_notification(
    ...
    notifications.send_user_change_email_notification(
        old_email, customer_user, get_plugins_manager(), channel_slug=channel_EUR.slug
    )
...
def test_send_password_reset_notification(
    mocked_notify, mocked_generator, is_staff, site_settings, customer_user, channel_EUR
):
    token = "token_example"
    ...
    mocked_notify.assert_called_once_with(
        expected_event, payload=expected_payload, channel_slug=channel_EUR.slug
    )
