...
class InvoiceQueryset(models.QuerySet):
    """
    Returns a queryset of invoices that are ready.

    Only includes invoices that are associated with jobs that have a status of SUCCESS.

    Returns:
        QuerySet: A queryset of invoices that are ready.
    """
    def ready(self):
...
class Invoice(ModelWithMetadata, Job):
    ...
    Invoices are generated by the system and can be downloaded by the customer.

    Attributes:
        order (Order): The order associated with the invoice.
        number (str): The invoice number.
        created (datetime): The date and time when the invoice was created.
        external_url (str): The external URL of the invoice.
        invoice_file (FileField): The file field for storing the invoice file.
    """
