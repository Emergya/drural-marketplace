...
import { authenticateGoogleUser } from "./queries";
...
export const SocialMediaLogin: React.FC = () => {
  ...
  const [checkGoogleUser] = useMutation(authenticateGoogleUser,{
    onCompleted(data){
      return data.token
    }
  });
  ...
  const googleLogin =  useGoogleLogin({
    onSuccess: async (tokenResponse )=> {
      ...
      try {
        ...
        const token = checkGoogleUser({
          variables: {googleId:userInfo.id}
        });
