...
import { getRemoveCheckoutJotInput } from "../../jobs/Checkout/utils";
import {
...
export class SaleorCheckoutAPI extends ErrorListener {
  ...
  removeCheckout = async (keepLines: boolean): CheckoutResponse => {
    const { id: checkoutId, lines: checkoutLines } =
      this.saleorState.checkout || {};

    if (checkoutId) {
      const removeCheckoutInput = getRemoveCheckoutJotInput(
        checkoutId,
        keepLines,
        checkoutLines
      );
    ...
    return {
      functionError: {
        error: new Error("You need to set a checkout berofe removing it."),
        type: FunctionErrorCheckoutTypes.CHECKOUT_NOT_SET,
      },
      pending: false,
    };
  };
