...
import {
  DeleteCheckout,
  DeleteCheckoutVariables,
} from "../../mutations/gqlTypes/DeleteCheckout";
import {
  CreatePaymentInput,
...
export class ApolloClientManager {
  ...
  signOut = async () => {
    await this.client.resetStore(); // cleans apollo store (cached) + refretch active queries
  };
  ...
  getRefreshedCheckoutLines = async (
    ...
    const linesWithMissingVariantUpdated = variants
      ? variants.edges.map(edge => {
          ...
          return {
            bookingId: existingLine?.bookingId || "",
            id: existingLine?.id,
  ...
  removeCheckout = async ({ checkoutId }: RemoveCheckoutInput) => {
    try {
      const { data, errors } = await this.client.mutate<
        DeleteCheckout,
        DeleteCheckoutVariables
      >({
        mutation: CheckoutMutations.deleteCheckoutMutation,
        variables: {
          checkoutId,
        },
      });
      ...
      if (data?.checkoutDelete?.errors.length) {
        return {
          error: data?.checkoutDelete?.errors,
        };
      }
      if (data?.checkoutDelete) {
        return {
          data: data.checkoutDelete,
        };
      }
      return {};
    } catch (error) {
