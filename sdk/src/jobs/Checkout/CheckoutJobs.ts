...
class CheckoutJobs extends JobsHandler<{}> {
  ...
  removeCheckout = async ({
    checkoutId,
    lines,
  }: RemoveCheckoutJobInput): PromiseCheckoutJobRunResponse => {
    const { data, error } = await this.apolloClientManager.removeCheckout({
      checkoutId,
    });

    this.localStorageHandler.setCheckout(lines ? { lines } : {});

    if (LocalStorageHandler.getPayment()) {
      this.localStorageHandler.setPayment({});
    }
    ...
    if (error) {
      return {
        dataError: {
          error,
          type: DataErrorCheckoutTypes.REMOVE_CHECKOUT,
        },
      };
    }
