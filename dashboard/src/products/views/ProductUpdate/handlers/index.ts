...
export function createUpdateHandler(
  ...
  return async (data: ProductUpdatePageSubmitData) => {
    const defaultChannel = allChannels[0].id;
    ...
    const productVariables: ProductUpdateVariables = {
      ...
      input: {
        ...
        bookableResources: data.bookableResources,
        category: data.category,
        categories: data.categories,
        chargeTaxes: data.chargeTaxes,
        collections: data.collections,
        description: getParsedDataForJsonStringField(data.description),
        details: getParsedDataForJsonStringField(data.details),
        duration: data.duration,
        hasNoPrice: data.hasNoPrice,
        name: data.name,
