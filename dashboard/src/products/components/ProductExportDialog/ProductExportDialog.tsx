...
const getInitialForm = (isStaff: boolean): ExportProductsInput => ({
  exportInfo: {
    attributes: [],
    channels: [],
    fields: [],
  ...
  scope: isStaff ? ExportScope.ALL : ExportScope.FILTER
});
...
const ProductExportDialog: React.FC<ProductExportDialogProps> = ({
  ...
  const initialForm = getInitialForm(user.isStaff);
      ...
      <>
        <DialogTitle>
          <FormattedMessage
            ...
            description="export services to csv file, dialog header"
          />
        </DialogTitle>
        <DialogContent>
          <ProductExportSteps
