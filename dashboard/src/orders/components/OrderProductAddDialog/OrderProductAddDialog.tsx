...
const OrderProductAddDialog: React.FC<OrderProductAddDialogProps> = props => {
  ...
  const getAvailableCompany = (product: SearchOrderVariant_search_edges_node) =>
    product.company.isEnabled && product.company.status === CompanyStatus.ACC;

  const getPubishProduct = (product: SearchOrderVariant_search_edges_node) =>
    !!product.channelListings.find(
      channelListing => channelListing.isPublished
    );

  const productChoices =
    products
      ?.filter(product => getValidProductVariants(product).length > 0)
      ?.filter(product => getAvailableCompany(product))
      ?.filter(product => getPubishProduct(product)) || [];
